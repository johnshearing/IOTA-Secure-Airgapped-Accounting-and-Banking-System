...
metadata._metadata.post = function(data, callback)
{
  ...
  // Behavior from meta.js at lefq4oks90h34rvcw8sg
  let optionValueKeyArray = ["field", "defaultElement", "option", "optionValue"]
  ...
  // Behavior from meta.js at lefq4oks90h34rvcw8sg
  let optionTextKeyArray = ["field", "defaultElement", "option", "optionText"]

  let optionTextArray = loadPayloadArray(optionTextKeyArray, data.payload);
  ...
  calculationNameArray.every(function(arrayElement)
  {
    if(typeof(arrayElement[1]) != 'string')
    {
      callback(400, {'Error' : 'calculationName must be of datatype string'});
      passedValidation = false;
      return false;  //Break this .every() loop
    }
    ...
    {
      callback(400, {'Error' : 'No calculationName was entered'}); 
      passedValidation = false;      
      return false;  //Break this .every() loop
    }
    else
  ...
  // Behavior from meta.js at lefq4oks90h34rvcw8sg
  let formElementTypeKeyArray = ["webPage", "form", "formElement", "formElementType"]

  let formElementTypeArray = loadPayloadArray(formElementTypeKeyArray, data.payload);
  // End of: Load the formElementTypeArray dynamically once the payload is known. 
  ...
  readInterface.on('close', function() 
  {
    ...
    _data.nextId(function(error, nextIdObject)
    {
      ...
      // Add any fields named "optionValueArray" to the object we will write to the database.
      metadataObject = buildBranches(optionValueArray, metadataObject);

      // Add any fields named "optionTextArray" to the object we will write to the database.
      metadataObject = buildBranches(optionTextArray, metadataObject);
      ...
      // Add any fields named "formElementTypeArray" to the object we will write to the database.
      metadataObject = buildBranches(formElementTypeArray, metadataObject);
...
metadata._metadata.put = function(data, callback)
{
  ...
  (
    ...
    &&  !optionValueArray.some(function(element){if(element){return true;}})
    &&  !optionTextArray.some(function(element){if(element){return true;}})
    ...
    &&  !formElementTypeArray.some(function(element){if(element){return true;}})
  ...
  _data.nextId(function(error, nextIdObject)
  {
    ...
    helpers.getMostRecent(dataObject, function(errorFromGetMostRecent, payload)
    {
      ...
      writable.write = function(payload)
      {
        ...
        for (let arrayIndex = 0; arrayIndex < optionValueArray.length; arrayIndex++) 
        {
          // Preprocessing for option.optionX.optionValue
          if(optionValueArray[arrayIndex]) // If the user supplied data for option.optionX.optionValue
          {
            // No preprocessing was specifed for option.optionX.optionValue. Use it as was supplied by the user.
          }
          else // If the user did not supply data for option.optionX.optionValue
          {
            // Save option.optionX.optionValue from the most recent record.
            metadataObject.option["option" + arrayIndex.toString()]["optionValue"] = optionValueArray[arrayIndex];
          }
        } 

        for (let arrayIndex = 0; arrayIndex < optionTextArray.length; arrayIndex++) 
        {
          // Preprocessing for option.optionX.optionText
          if(optionTextArray[arrayIndex]) // If the user supplied data for option.optionX.optionText
          {
            // No preprocessing was specifed for option.optionX.optionText. Use it as was supplied by the user.
          }
          else // If the user did not supply data for option.optionX.optionText
          {
            // Save option.optionX.optionText from the most recent record.
            metadataObject.option["option" + arrayIndex.toString()]["optionText"] = optionTextArray[arrayIndex];
          }
        } 
        ...
        for (let arrayIndex = 0; arrayIndex < formElementTypeArray.length; arrayIndex++) 
        {
          // Preprocessing for formElement.formElementX.formElementType
          if(formElementTypeArray[arrayIndex]) // If the user supplied data for formElement.formElementX.formElementType
          {
            // No preprocessing was specifed for formElement.formElementX.formElementType. Use it as was supplied by the user.
          }
          else // If the user did not supply data for formElement.formElementX.formElementType
          {
            // Save formElement.formElementX.formElementType from the most recent record.
            metadataObject.formElement["formElement" + arrayIndex.toString()]["formElementType"] = formElementTypeArray[arrayIndex];
          }
        } 
        ...
        function(payloadKeyArrayElement, payloadKeyArrayIndex)
        {
          ...
          else if(payloadKeyArrayIndex != payloadKeyArray.length - 1) 
          {
            if(
                ...
                && splitExamElementArray[payloadKeyArrayIndex * 2] != "defaultElement"
                && splitExamElementArray[payloadKeyArrayIndex * 2] != "validation"
                && splitExamElementArray[payloadKeyArrayIndex * 2] != "default"                 
              )
            {
              passExamElement = false;
            }
