...
`          
        ...
        for (let fieldKey in fieldsObject)
        {
          ...
          else // This top level field is not an object - It's just a regular data field.
          {
            ...
            {
              // Get any calculations from the data dictionary if any.
              // Stop (break) when we find the one we are looking for.
              for (let calcKey in fieldsObject[fieldKey].calculation)
              {
                if(calcKey === "postHandlerPreprocessing")
                {
                  fieldCalculationCode = fieldsObject[combinedDefaultElementsObject[fieldKey].parentField].calculation[calcKey]; 
                  break;           
                }
              }
            }   
              ...
              { 
                ...
                for (let property in Address[objKey].properties)
                {
                  ...
                  { 
                    ...
                    // Loop through defaultElementsObject.  
                    // ???? The code below will not work if there is more than one defaultElement in 
                    // the object because it will write the same thing for all of them.
                    // Probably need to build objectPath inside the for in loop below.
...
`
        ...
        // The code above needs modification. The code below should be deleted.  ????
        ...
        for (let key in combinedDefaultElementsObject) 
        {
          ...
          // Start of: Insert code for handling calculated fields and field preprocessing
          // Loop thorough the top level fields object and drill in as required.
          for (let fieldKey in fieldsObject)
          {
            // If this top level field is an object containing more fields inside...
            if(fieldsObject[fieldKey].properties)
            {
              ...
              {      
                // if the subfield is also an object then this function calls itself using the subfield parameters.
                // This will continue drilling in until we are at the end of the branches where we encounter data fields
                if(Address[objKey].properties)
                { 
                  // Check each property of the subobject to see if it too is an object.
                  for (let property in Address[objKey].properties)
      ...
      writable.write = function(payload)     
      {
        ...
        for (let key in editPageFormsObject) 
        {
          ...
          else if (key != "deleteForm") 
          {
            ...
            //???? Made changes to function. Test if this still works by building userEdit
