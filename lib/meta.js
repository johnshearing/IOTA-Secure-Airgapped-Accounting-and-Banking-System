...
`
        ...
        for (let fieldKey in combinedDefaultElementsObject) 
        {
          ...
          for (let validationKey in combinedDefaultElementsObject[fieldKey].validation) 
          {
            // console.log("3. Below is the value of combinedDefaultElementsObject[fieldKey].validation[", validationKey, "] : \n" , combinedDefaultElementsObject[fieldKey].validation[validationKey], "\n\n");

            if(validationKey === "passIfNotEmpty")
            {
              // If the user supplied the validation string
              if(combinedDefaultElementsObject[fieldKey].validation[validationKey])
              {
                htmlString = htmlString + 
                "  // " + validationKey + "\n" +
                "  " + combinedDefaultElementsObject[fieldKey].validation[validationKey] + "\n" +
              ...
              else // No user supplied validation. Create a generic validation string.
              {
                htmlString = htmlString + 
                "  // passIfNotEmpty" + "\n" +
                "  if(!" + fieldKey + "){return callback(400, {'Error' : 'No " + fieldKey + " was entered'});}" + "\n" +
                "\n";
              }
            }
            else if(validationKey === "passIfString")
            {
              // If the user supplied the validation string
              if(combinedDefaultElementsObject[fieldKey].validation[validationKey])
              {
...
app.load${recordObject.table.tableName}EditPage = async function()
{
  ...
  // Run the query defined below to get the email and timeStamp from the ${recordObject.table.tableName}ID.    
  ...
  // Define a function to get the email and the timeStamp from the ${recordObject.table.tableName}ID
  ...
  // End of: Define a function to get the email and the timeStamp from the ${recordObject.table.tableName}ID
