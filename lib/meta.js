...
function renumberNamedElementsInGroupWrapper(groupWrapper)
{
  ...
  // Prevents pushing more than one element onto the ansestorWrappers array or incrementing 
  // the element value more than once for each loop through an elementWrapper.
  let pushedOrIncremented = false;
  ...
  {
    // Prevents incrementing the counter element in ansestorWrappers array more than once for each loop through the elementWrapper.
    ...
    {
      //  only push once                prevents error           if current wrapper is inside the previous
      if(pushedOrIncremented === false && previousElementWrapper && previousElementWrapper.contains(elementWrapper))
      {
      ...
      else // current wrapper is not inside the previous
      {
        ...
        while (true)
        {
          ...
          else if(ancestorWrappers.length > 1 && ancestorWrappers[ancestorWrappers.length - 1][0].contains(elementWrapper))
          {
            ...
            if(pushedOrIncremented === false && incremented === false)
            {
