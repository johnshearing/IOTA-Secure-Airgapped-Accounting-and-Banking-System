...
// Define a function which inserts routes and dependencies into server.js for interacting with tables.
// server.js is completely built by this function so any changes to server.js must be made here and then
...
meta.makeAddPageElementHtml = function(elementName, recordObject)
{
  ...
  if(recordObject.webPages.addPage.forms.addForm.formElements[elementName].properties)
  {
    ...
    function drillIntoObject(formElementObj, objKey, previousObjKey, previousKeyPart, fieldsObject, objectNestLevel)
    {      
      // if the subfield is also an object then this function calls itself using the subfield parameters.
      // This will continue drilling in until we are at the end of the branches
      ...
      { 
        ...
        for (let property in formElementObj[objKey].properties)
        {
          ...
          if(formElementObj[objKey].properties[property].properties)
          {
            ...
            // This function calls itself so as to go deeper into the object
            drillIntoObject(formElementObj[objKey].properties, property, objKey, "properties", fieldsObject[objKey]["properties"], objectNestLevel + 1);
