...
app.load${recordObject.tableName}EditPage = async function()
{
  ...
  for (let key in recordObject)
  {
    ...
    if(recordObject[key].subObject)
    {
      ...
      function drillIntoObject(address, objKey, objectNestLevel, previousPath)
      {
        // If the sub object we are currently examining contains another sub object...
        // Then get setup for a recursive call so as to drill deeper into the recordObject from the server.
        // Continue drilling in until we are at the end of each branch where we find data to load onto the form. 
        if(address[objKey].subObject)
        { 
          // Check each property of the subobject to see if it too is an object.
          for (let property in address[objKey].subObject)
          {
            ...
            { 
              ...
              let finalBranch = address[objKey].subObject;
