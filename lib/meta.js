...
/ The underscore and the question mark ( _  ?)  may not be used in table names nor in field names.
/ 
/
*/
...
{
  // Define the function that fires when the addNewButton is selected.
  function onClickBehaviorForAddNewButton (event)  
  {
    // Stop it from redirecting anywhere
    event.preventDefault();

    let elementWrapperCount = document.querySelectorAll(".elementWrapper").length;

    // Deep clone a new elementWrapper from the previous and append it to the DOM
    let elmnt = document.querySelectorAll(".elementWrapper")[elementWrapperCount - 1];
    ...
    // Create an event listener for the onchange event of fields in this clone.
    // cln.querySelectorAll(".conjunctionSelector")[0].addEventListener("change", onChangeBehaviorForConjunctionSelector);

    // Append above the addNewButton
    let targetGroupWrapper = document.querySelectorAll(".groupWrapper")[0];
    targetGroupWrapper.insertBefore(cln, targetGroupWrapper.childNodes[targetGroupWrapper.childNodes.length -2]);


  } // End of: function onClickBehaviorForAddNewButton (event)
  // End of: Define the function that fires when the addNewButton is pressed.

  // Bind the function above to the onClick event of the addNewButtons.
  document.querySelectorAll('.addNewButton').forEach(function(currentButton)
  {
    currentButton.addEventListener("click", onClickBehaviorForAddNewButton);
  });
      ...
      { 
        ...
        for (let property in formElementObj[objKey].properties)
        {
          ...
          if(formElementObj[objKey].properties[property].properties)
          {
            ...
            elementString = elementString + "  ".repeat(objectNestLevel) +
            "    <button type=\"button\" class=\"modObjects green addNewButton\">Add New</button>"  + "\n";          
          }
