...
// ??? Need to handle multiple fields. 
// ??? Need to handle multiple unique fields.
// ??? Need to restructure code so that validation code and code for calculated fields call following code as a function.
// ??? This keeps indentation correct.
// ??? Need to handle the unique field.
        ...
        {
          console.log("2. This is combinedDefaultElementsObject[" + key + "]: " + "\n",combinedDefaultElementsObject[key], "\n");
        }        
...
${recordObject.table.tableName}._${recordObject.table.tableName}.post = function(data, callback)
{
  ...
  if(email && password)
  {
    // Will toggle this to false if we find the email already exists in the ${recordObject.table.tableName} table.
    ...
    // ${recordObject.table.tableName} table and compare with the email provided.
    ...
    readInterface.on('line', function(line) 
    {
      ...
      // Several different record sets with the supplied email and the same ${recordObject.table.tableName}Id 
      ...
      // be several sets of records in the ${recordObject.table.tableName} table currently that have the same email 
      // and the same ${recordObject.table.tableName}Id.
      ...
      // In this table, the email is a unique key as well as the ${recordObject.table.tableName}Id.
      ...
      // In that case, the ${recordObject.table.tableName}Id will be different but the email will be the same.         
      ...
      if (email == lineObject.email) // we found a matching entry
      {
        if (lineObject.deleted == false) // The record has not been deleted so it's a duplicate. Not unique.
        {
          ...
          uniqueIdOfRecordHoldingCandidateEmail = lineObject.${recordObject.table.tableName}Id;
        }
      ...
      else if(emailIsUnused === false && uniqueIdOfRecordHoldingCandidateEmail === lineObject.${recordObject.table.tableName}Id)
      {
                  ...
                  if (!err)  // The history file has been appended to successfully.
                  {
                    ...
                    (
                      ...
                      function(err)
                      {
                          ...
                          // start of callback code which is run after attempting to remove the lock.
                          {
                            ...
                            else // Good write but unable to remove lock on database.
                            {
                              ...
                              (
                                7,
                                '${helpers.createRandomString(20)}' + '\\n' +
                                'Successful write to ${recordObject.table.tableName} table but unable to remove lock on database' + '\\n' +
                              ...
                              callback(500, {'Error' : 'Successful write to ${recordObject.table.tableName} table but unable to remove lock on database'});
                        ...
                        else // There was an error appending to ${recordObject.table.tableName} table.
                        {
                          helpers.log // Log the error.
                          (
                            5,
                                  ...
                                  function(err)
                                  {
                                    ...
                                    else // There was an error when rolling back record for ${recordObject.table.tableName}.
                                    {
                                      ...
                                      (
                                        ...
                                        JSON.stringify(logObject) + '\\n' +   
                                        'An error here does not necessarily mean the deleteing append to ${recordObject.table.tableName} did not happen.' + '\\n' +                                        
                          ...
                          callback(500, {'Error' : 'Could not create the new ${recordObject.table.tableName} record.'});
...
app.load${recordObject.table.tableName}EditPage = async function()
{
  ...
  // Run the query defined below to get the email address and timeStamp from the ${recordObject.table.tableName}ID.    
  ...
  // Define a function to get the email address and the timeStamp from the ${recordObject.table.tableName}ID
  ...
  // End of: Define a function to get the email address and the timeStamp from the ${recordObject.table.tableName}ID
