...
/ The underscore ( _ )  may not be used in table names nor in field names.
/ 
/
*/
...
`
        ...
        // First make an array of top level field names from the record object.
        let topFieldsArray = Object.keys(recordObject.fields); 

        // Loop thorough the top level fields
        topFieldsArray.forEach(function (elementValue, topFieldsArrayIndex)
        {
          // If this top level field is an object containing more fields inside...
          if(recordObject.fields[elementValue].properties)
          {
            ...
            drillIntoObject(recordObject.fields, topFieldsArray, objectNestLevel, "")
          }
          else // This top level field is not an object - It's just a regular field.
          {
            ...
            keysOfFieldsArray.push(topFieldsArray[topFieldsArrayIndex]);            
          }

        }) // End of: topFieldsArray.forEach(function (elementValue, topFieldsArrayIndex){...}
        ...
        // Loop thorough the top level fields
        topFieldsArray.forEach(function (elementValue)
        {
          // If this top level field is an object containing more fields inside...
          if(recordObject.fields[elementValue].properties)
          {
            // Declare a function that we will use to drill into the object.
            // function drillIntoObject(Address, objKey, previousObjKey, previousKeyPart, previousPath, objectNestLevel)
            ...
            {      
              ...
              { 
                ...
                for (let property in Address[objKey].properties)
                {
                  ...
                  {
                    // This function calls itself so as to go deeper into the object
                    // drillIntoObject(Address[objKey].properties, property, objKey, "properties", previousPath + objKey + "_properties_", objectNestLevel + 1);
          ...
          {
            // Check if there is a calculation on this field.
            // If there is then we will not have the value until after the calculation has been made.
            // So set the value to an empty string.
...
`
        ...
        // This code handles the possibility that no fields are required to be unique.
        ...
        // I intend to build a simple beautifier into the generator.
...
`
  ...
  let ${uniqueFieldsArray[0]}_IsUnused = true;
  ...
  let uniqueIdOfRecordHoldingCandidate_${uniqueFieldInTitleCase} = false; 
  ...
  readInterface.on('line', function(line) 
  {
    ...
    if (${uniqueFieldsArray[0]} == lineObject.${uniqueFieldsArray[0]}) // we found a matching entry
    {
      ...
      {
        ${uniqueFieldsArray[0]}_IsUnused = false; // This flag used in the on close event listener below. 
        ...
        // 1. Set the ${uniqueFieldsArray[0]}_IsUnused flag to true again
        // 2. clear out the variable tracking the uniqueId of the record.
        uniqueIdOfRecordHoldingCandidate_${uniqueFieldInTitleCase} = lineObject.${recordObject.table.tableName}Id;
      }
      // The matching record we found has been deleted so it may as well not exist. The new record is still unique.
      else 
      {
        ${uniqueFieldsArray[0]}_IsUnused = true;
      } 
    ...
    else if(${uniqueFieldsArray[0]}_IsUnused === false && uniqueIdOfRecordHoldingCandidate_${uniqueFieldInTitleCase} === lineObject.${recordObject.table.tableName}Id)
    {
  ...
  readInterface.on('close', function() 
  {
    ...
    if (!${uniqueFieldsArray[0]}_IsUnused) 
    {      
      helpers.log
      (
        5,
        '${helpers.createRandomString(20)}' + '\\n' +
        'The ${uniqueFieldsArray[0]} : ' + ${uniqueFieldsArray[0]} + ' already exists' + '\\n'                                  
      ); // End of: helpers.log(...)
...
`          
        ...
        // Start of: Insert code for handling calculated fields and field preprocessing
        // Loop thorough the top level fields
        topFieldsArray.forEach(function (elementValue)
        {
          // If this top level field is an object containing more fields inside...
          ...
          {
            // Declare a function that we will use to drill into the object.
            function drillIntoObject(Address, objKey)
            {      
              // if the subfield is also an object then this function calls itself using the subfield parameters.
              // This will continue drilling in until we are at the end of the branches
              if(Address[objKey].properties)
              { 
                ...
                for (let property in Address[objKey].properties)
                {
                  ...
                  {
                    // This function calls itself so as to go deeper into the object
                    drillIntoObject(Address[objKey].properties, property);
                  }
                  else // This property is not an object. We are at the end of the branch where we encounter a formElement.
                  {
                    if(Address[objKey].properties[property].published === true)
                    {
                      // Get any calculations from the data dictionary if any.
                    ...
                    } // End of: If the field is published
                  } // End of: else - This property is not an object.
                } // End of: for-let Check each property of the subobject to see if it too is an object.
              } // End of: if the subfield is also an object
            }; // End of: function drillIntoObject(obj, objKey, previousObjKey, previousKeyPart){...}
            ...
            // Call the function defined above. 
            // This function will call itself several times if drilling into nested objects is required.
            drillIntoObject(recordObject.fields, topFieldsArray)
          }
          else // This top level field is not an object - It's just a regular field.
          {
            if(recordObject.fields[elementValue].published === true)
            {
              // Get any calculations from the data dictionary if any.
              for (let calcKey in recordObject.fields[elementValue].calculation)
              {
                if(calcKey === "postHandlerPreprocessing")
                {
                  fieldCalculationCode = recordObject.fields[combinedDefaultElementsObject[key].parentField].calculation[calcKey]; 
                  break;           
                }
              }
            }   
...
`
    ...
    ${fieldCalculationCode}`
        ...
        // End of: Insert code for handling calculated fields and field preprocessing
...
`
        ...
        topFieldsArray.forEach(function (elementValue)
        {
            ...
            { 
              ...
              { 
                ...
                for (let property in Address[objKey].properties)
                {
                  ...
                  else // This property is not an object. We are at the end of the branch where we encounter a field.
                  { 
                    // Assemble the path through the current object to the current field.
                    let objectPath = objKey + "_" + "properties" + "_" + property;
                    ...
                    htmlString = htmlString +
                    // "      " + recordObject.table.tableName + "Object" + "." + elementValue + ".properties." + property + " = " + objectPath + ";" + "\n";
          ...
          {  
            // We will likely need to change the following line as guided by testing with metadata record 1285. ????
            htmlString = htmlString +
...
`      
      ...
      ${recordObject.table.tableName}deleted = false;
...
`  {
  ...
  _data.nextId(function(error, nextIdObject)
  {
    ...
    let ${recordObject.table.tableName}Object = 
    {
      "${recordObject.table.tableName}Id" : ${recordObject.table.tableName}Id,
`
              ...
              { 
                ...
                for (let property in Address[objKey].properties)
                {
                  ...
                  { 
                    ...
                    if(Address[objKey].properties[property].calculation)
                    {
                      htmlString = htmlString +  
                      "      \"" + objectPath + "\" : \"\" ," + "\n";
                    }
...
`
        ...
        }); // End of: uniqueFieldsArray.forEach(function (elementValue, elementIndex, keysOfFieldsArray)
...
`
        ...
        // Loop through the combinedDefaultElementsObject to create if else 
        // statements which check if the user supplied data for those elements.

        // Start of: Insert code for handling calculated fields and field preprocessing
        for (let key in combinedDefaultElementsObject) 
        {
          htmlString = htmlString + 
          "        // Preprocessing for " + key + "\n" +
          "        if(" + key + ") // If the user supplied data for " + key + "\n" +
          "        {" + "\n";

          let parentField = combinedDefaultElementsObject[key].parentField;

          let fieldCalculationCode = false;         

          // Get any calculations from the data dictionary if any.
          // The calculation will be on the parent field.
          for (let calcKey in recordObject.fields[combinedDefaultElementsObject[key].parentField].calculation)
          {
            if(calcKey === "putHandlerPreprocessing")
            {
              fieldCalculationCode = recordObject.fields[combinedDefaultElementsObject[key].parentField].calculation[calcKey];
              // console.log("This is the fieldCalculationCode:\n", fieldCalculationCode, "\n")   
              break;           
            }
          }
          
          if(fieldCalculationCode) // If the programmer entered code in data dictionary.
          {
            htmlString = htmlString + "    " + fieldCalculationCode + "\n" +
            "          // Saving calculation on " + key + " supplied by the user to " + parentField + "\n" +
            "          userObject." + parentField + " = " + parentField + ";" + "\n\n" +
            "        } // End of: If the user supplied data for " + key + "\n" +
            "        else // If the user did not supply data for " + key + "\n" +
            "        {" + "\n" + 
            "          // saving " + parentField + " from the most recent record." + "\n" +
            "          " + recordObject.table.tableName + "Object." + parentField + " = recordObject." + parentField + ";" + "\n" +
            "        }" + "\n\n";
          }
          else // No code in the data dictionary for this process. So build generic code.
          {
            htmlString = htmlString +
            "          // No preprocessing was specifed for " + key + ". Use it as was supplied by the user." + "\n" +
            "        }" + "\n" +
            "        else // If the user did not supply data for " + key + "\n" + 
            "        {" + "\n" +
            "          // Save " + parentField + " from the most recent record." + "\n" +
            "          " + recordObject.table.tableName + "Object." + parentField + " = recordObject." + parentField + ";" + "\n" +
            "        }" + "\n";
          }
        } // End of: for (let key in combinedDefaultElementsObject){...}
        // End of: Insert code for handling calculated fields and field preprocessing
  ...
  readInterface.on('line', function(line) 
  {
    ...
    if(recordWasDeleted === false)
    {
      `
        ...
        {
          if(recordObject.fields[elementValue].published === false)
          {
            unpublishedFieldsArray.push(elementValue);   
          }
        });
        // console.log("5. These fields are not published: ", unpublishedFieldsArray, "\n\n");  
...
*/
        ...
        (
          recordObject.table.directory, 
          recordObject.table.tableName + "LogicTest", 
          fileExtension,
          htmlString, 
          function(err)
          {
      ...
      writable.write = function(payload)     
      {
        ...
        for (let fieldKey in fieldsObject) 
        {
          ...
          {    
            ...
            // There may be more than one default element for each field 
            // (Consider velocity calculated from both time and distance elements.) 
            // Another situation is the table field in metadata.json. 
            // The value of this field is an object which contains several default elements.

            // The field might be an object which contains subfields each of which must be 
            // handled taking into account the considerations above.
            // The code below takes care of all of this.

            // If the field is an object we will need to create dot notation in order to refer to the field.
            if(fieldsObject[fieldKey].dataType === "object")
            {
              ...
              function drillIntoObject(obj, objKey, startingPath)
              {
                ...
                {
                  propertyPath = startingPath + objKey + "." + keyPart;                                

                  // if the subfield is also an object then this function calls itself using the subfield parameters.
                  // This will continue drilling in until we are at the end of the branches and
                  if(objValue[objKey].properties[keyPart].dataType === "object")
                  { 
      ...
      { 
        ...
        for (let property in formElementObj[objKey].properties)
        {
          ...
          { 
            ...
            // console.log("12. This is the formElementObj.properties[", property, "]\n", formElementObj[objKey].properties[property], "\n\n");
