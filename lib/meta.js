...
meta.makeAddPageElementHtml = function(elementName, recordObject)
{
  ...
  if(recordObject.webPages.addPage.forms.addForm.formElements[elementName].properties)
  {
    ...
    function drillIntoObject(obj, objKey, previousObjKey, previousKeyPart)
    {                
      // Here is where we do the drilling.
      for (let keyPart in obj[objKey]) 
      {
        // if the subfield is also an object then this function calls itself using the subfield parameters.
        // This will continue drilling in until we are at the end of the branches
        if(obj[objKey].properties)
        { 
          // Check each property of the subobject to see if it too is an object.
          for (let property in obj[objKey].properties)
          {
            // If the property is also an object
            if(obj[objKey].properties[property].properties)
            {
              // This function calls itself so as to go deeper into the object
              drillIntoObject(obj[objKey].properties, property, objKey, keyPart);
            }
            else // This property is not an object. We are at the end of the branch where we encounter a formElement.
            {
              // If at least one level deep.
              if(previousObjKey.length > 0)
              { 
                // Return the entire path including parent objects.
                console.log(`This is the object path: ${previousObjKey}.${previousKeyPart}.${objKey}.${keyPart}.${property}`)
                console.log(); 
              }
              else // We are at the top of an object. There are no parent objects to include in the path.
              {
                console.log(`This is the object path: ${objKey}.${keyPart}.${property}`)
                console.log();  
              } // End of: If/Else Nest level.
    ...
    drillIntoObject(recordObject.webPages.addPage.forms.addForm.formElements, elementName, "", "")
    ...
    elementString = elementString + 
    "        <div class=\"inputLabel\">" + "someTitle" + "</div>" + "\n";


    elementString = elementString + 
    "        " + "<input name=\"email\" type=\"text\" placeholder=\"alice@gmail.com\"/>" + "\n" +
    "      </div>"  + "\n";
