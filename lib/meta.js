              ...
              { 
                ...
                for (let property in address[objKey].subObject)
                {
                  ...
                  if(address[objKey].subObject[property].subObject)
                  {
                    ...
                    drillIntoObject(address[objKey].subObject, property, objectNestLevel + 1, previousPath + objKey + "_subObject_");
                  }
            ...
            {      
              ...
              { 
                ...
                for (let property in address[objKey].subObject)
                {
                  ...
                  {
                    // This function calls itself so as to go deeper into the object
                    drillIntoObject(address[objKey].subObject, property, previousPath + objKey + "_subObject_", objectNestLevel + 1);
                  }
                  else // This property is not an object. We are at the end of the branch where we encounter a data field.
                  { 
                    // Assemble the path through the current object to the current data field.
                    ...
                    let defaultElementsObject = address[objKey].subObject[property].defaultElements;
              ...
              { 
                ...
                if(objectNestLevelHistory === objectNestLevel)
                {
                  if(objectNestLevel === 1)
                  {
                    ...
                    persistArray.push([objKey + ".subObject", "", "container"])
                  }
                  else
                  {
                    persistArray.push([subObjectPath + "." + objKey, "", "container"])
                    persistArray.push([subObjectPath + "." + objKey + ".subObject", "", "container"])
                  }

                  if(objectNestLevel === 1)
                  {
                  // Note that we are going deeper into the object.
                  subObjectPath = objKey + ".subObject";
                  }
                  else
                  {
                  // Note that we are going deeper into the object.
                  subObjectPath = subObjectPath + "." + objKey + ".subObject";
                  }                  
                ...
                for (let property in address[objKey].subObject)
                {
                  ...
                  { 
                    ...
                    (
                      ...
                      address[objKey].subObject[property].defaultElements 
                      // And it is populated with defaultElements
                      && Object.keys(address[objKey].subObject[property].defaultElements).length > 0
                      // And one of the defaultElements has the same name as the field...
                      && address[objKey].subObject[property].defaultElements.hasOwnProperty(property)
                    )
                    {
...
`
        ...
        ${recordObject.tableName}Object.${containerPath}["${containerName}" + arrayIndex.toString()].subObject = {};` 
          ...
          bottomLevelDatafieldArray.forEach(function(element)
          {
            ...
            htmlString = htmlString + 
`
        ${recordObject.tableName}Object.${containerPath}["${containerName}" + arrayIndex.toString()].subObject["${dataFieldName}"] = ${dataFieldName}Array[arrayIndex];`        
              ...
              { 
                ...
                for (let property in address[objKey].subObject)
                {
                  ...
                  { 
                    ...
                    if(address[objKey].subObject[property].calculation && Object.keys(address[objKey].subObject[property].calculation).length > 0)
                    {
                      ...
                      for (let calcKey in address[objKey].subObject[property].calculation)
                      {
                        if(calcKey === "postHandlerPreprocessing")
                        {
                          fieldCalculationCode = address[objKey].subObject[property].calculation[calcKey]; 
                          break;           
                        }
                      } 
                  ...
                  { 
                    ...
                    if(address[objKey].subObject[property].calculation && Object.keys(address[objKey].subObject[property].calculation).length > 0)
                    {
                      ...
                      else // No code in the data dictionary for this process. So build generic code.
                      {
                        ...
                        // Removes the subObject from the xPath
                        let xPrepath = objectPath.split("0")[0].split(".").slice(0, -1).join(".");
...
`        for (let arrayIndex = 0; arrayIndex < ${fieldName}Array.length; arrayIndex++) 
        {
          ...
          else // If the user did not supply data for ${xPath}
          {
            ...
            ${recordObject.tableName}Object.${xPrepath}["${containerX}" + arrayIndex.toString()].subObject["${fieldName}"] = ${fieldName}Array[arrayIndex];
          }
        } 
...
function renumberNamedElementsInGroupWrapper(groupWrapper)
{
  ...
  // Example fieldName: field_subObject_field0_subObject_defaultElement_subObject_defaultElement0_subObject_elementName  
    ...
    elementWrapper.querySelectorAll('.resetElement').forEach(function(fieldElement)
    {
      ...
      else // current wrapper is not inside the previous
      {
        ...
        while (true)
        {
          ...
          else if(ancestorWrappers[ancestorWrappers.length - 1][0].contains(elementWrapper))
          {
        ...
        } // End of: while (ancestorWrappers.length > 1)
      ...
      // The following regular expression will find numbers in a string
      let searchExpression = /d+/g
        ...
        function (accumulator, currentSplitElement, currentSplitElementIndex) 
        {
          ...
          if(hasANumber(currentSplitElement))
          {
            ...
            if(encountersWithNumbers === ancestorWrappers.length)
            {
              ...
              accumulator = accumulator +  currentSplitElement.replace(/d+/g, ancestorWrappers[ancestorWrappers.length - 1][1]);
              ...
              { 
                ...
                for (let property in address[objKey].subObject)
                {
                  ...
                  { 
                    ...
                    if(address[objKey].subObject[property].published === true)
                    {                      
...
`
                    } // End of: if(address[objKey].subObject[property].published === true){...}
                  } // End of: else - This property is not an object.
                } // End of: for-let Check each property of the subobject to see if it too is an object.
              } // End of: if the subfield is also an object
            }; // End of: function drillIntoObject(address, objKey, objectNestLevel, previousPath){...}
...
meta.makeAddPageElementHtml = function(elementName, recordObject)
{
  ...
  if(recordObject.webPages.addPage.forms.addForm.formElements[elementName].subObject)
  {
    ...
    {      
      ...
      if(formElementObj[objKey].subObject)
      { 
        // Check each property of the subobject to see if it too is an object.
        for (let property in formElementObj[objKey].subObject)
        {

          // If the property is also an object.
          // We can tell because objects in this system have a property called "subObject"
          if(formElementObj[objKey].subObject[property].subObject)
          {

            // A label on numbered objects interferes with the operation of the up and down controls.
            // Checking that a label exists for this property and that the property key does not contain a number.
            if(formElementObj[objKey].subObject[property].labelText && !helpers.hasANumber(property))
            {
              // objectNestLevel handles indentation as we make HTML for the addPage. 
              elementString = elementString +
              "  ".repeat(objectNestLevel) + "    <div class=\"objectLabel\">" + formElementObj[objKey].subObject[property].labelText + "</div>" + "\n";
            }
            ...
            // This function calls itself so as to go deeper into the object
            drillIntoObject(formElementObj[objKey].subObject, property, fieldsObject[objKey]["subObject"], objectNestLevel + 1, previousPath + objKey + "_subObject_");

            // If stripping the last character presumably a zero from property gives the objectKey then it's time to make buttons.
            if(property.slice(0,-1) ===  objKey)
            {
              elementString = elementString + 
          ...
          { 
            ...
            let defaultElementsObject = fieldsObject[objKey].subObject[property].defaultElements[property];

            // console.log("11. This is the defaultElementObject\n", defaultElementsObject, "\n\n");
          
            // Deep merge the defaultElements with formElements - formElements taking precedence.
            let mergedElements = extend(true, defaultElementsObject, formElementObj[objKey].subObject[property]);
    ...
    if(recordObject.webPages.editPage.forms[key].formElements[elementValue].subObject)
      ...
      {      
        ...
        { 
          ...
          {
            // If the property is also an object...
            // We will know because objects contain a property called "subObject"
            if(formElementObj[objKey].subObject[property].subObject)
            {
              // Make the HTML for a container to hold this object's subObject.
              // objectNestLevel handles indentation as we make HTML for the addPage.
              elementString = elementString + 
