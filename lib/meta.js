...
/ Numerals may not be used in field names except to append o zero onto the field names of
/ objects which are intended to be duplicated.
/ For instance phone0 in the contact table when a contact may have many phone numbers.
/ Do not use any punctuation in table and field names. Only A to Z or a to z
/
*/
...
`
        ...
        for (let fieldKey in fieldsObject)
        {
          ...
          if(fieldsObject[fieldKey].properties)
          {
            ...
            {      
              ...
              if(address[objKey].properties)
              { 
                // Check each property of the subobject to see if it too is an object.
                for (let property in address[objKey].properties)
                {
                  // If the property is also an object
                  if(address[objKey].properties[property].properties)
                  {
                    // This function calls itself so as to go deeper into the object
                    drillIntoObject(address[objKey].properties, property, objectNestLevel + 1, previousPath + objKey + "_properties_");
                  }
                  ...
                  { 
                    ...
                    let objectPath = objKey + "_" + "properties" + "_" + property;
        ...
        for (let fieldKey in fieldsObject)
        {
          ...
          if(fieldsObject[fieldKey].properties)
          {
            ...
            {      
              // if the subfield is also an object then this function calls itself using the subfield parameters.
              // This will continue drilling in until we are at the end of the branches where the fields are found.
              if(address[objKey].properties)
              { 
                // Check each property of the subobject to see if it too is an object.
                for (let property in address[objKey].properties)
                {
                  ...
                  {
                    // This function calls itself so as to go deeper into the object
                    drillIntoObject(address[objKey].properties, property, previousPath + objKey + "_properties_", objectNestLevel + 1);
                  }
                  else // This property is not an object. We are at the end of the branch where we encounter a data field.
                  { 
                    // Assemble the path through the current object to the current data field.
                    ...
                    let defaultElementsObject = address[objKey].properties[property].defaultElements;
                ...
                } // End of: for (let property in address[objKey].properties){Check each property of the subobject to see if it too is an object.}
              } // End of: if the subfield is also an object
...
`                  
  ...
  {
    if(typeof(arrayElement) != 'string'){return callback(400, {'Error' : '${lastPart} must be of datatype string'});}
  });
...
`                  
  ...
  {
    if(typeof(arrayElement) != 'string'){return callback(400, {'Error' : '${lastPart} must be of datatype string'});}
    if(!arrayElement || arrayElement.trim().length === 0){return callback(400, {'Error' : 'No ${lastPart} was entered'});}else{arrayElement = arrayElement.trim()}
  });
  // End of: Validate elements in the ${lastPart}Array  
`
            ...
            {      
              ...
              { 
                ...
                for (let property in address[objKey].properties)
                {
                  ...
                  {
                    // This function calls itself so as to go deeper into the object
                    drillIntoObject(address[objKey].properties, property, objKey, "properties", objectNestLevel + 1);
                  }
                  else // This property is not an object. We are at the end of the branch where we encounter a data field.
                  { 
                    // Assemble the path through the current object to the current data field.
                    let objectPath = objKey + "." + "properties" + "." + property;
                    ...
                    if(address[objKey].properties[property].unique === true)
                    {
...
`
  ...
  readInterface.on('line', function(line) 
  {
    ...
    // The ${recordObject.table.tableName}Id also serves as the primary key.
    // The difference is that the ${recordObject.table.tableName}Id may never change whereas the ${uniqueFieldsArray[0]}
    ...
    if (${uniqueFieldsArray[0].split(".").join("_")} == lineObject.${uniqueFieldsArray[0]}) // we found a matching entry
    {
      if (lineObject.deleted == false) // The record has not been deleted so it's a duplicate. Not unique.
      {
        ...
        uniqueIdOfRecordHoldingCandidate_${uniqueFieldInTitleCase.split(".").join("_")} = lineObject.${recordObject.table.tableName}Id;
      }
    ...
    else if(${uniqueFieldsArray[0].split(".").join("_")}_IsUnused === false && uniqueIdOfRecordHoldingCandidate_${uniqueFieldInTitleCase.split(".").join("_")} === lineObject.${recordObject.table.tableName}Id)
    {
...
`          
    ...
    _data.nextId(function(error, nextIdObject)
    {
      ...
      let ${recordObject.table.tableName}Object = {};
      ${recordObject.table.tableName}Object.${recordObject.table.tableName}Id = nextIdObject.nextId;
...
`
        ...
        for (let fieldKey in fieldsObject)
        {
            ...
            { 
              ...
              { 
                ...
                if(objectNestLevelHistory === objectNestLevel)
                {
                  ...
                  {
                    persistArray.push([objKey, "", "container"])
                    persistArray.push([objKey + ".properties", "", "container"])
                  }
                  else
                  {
                    persistArray.push([propertiesPath + "." + objKey, "", "container"])
                    persistArray.push([propertiesPath + "." + objKey + ".properties", "", "container"])
                  }

                  if(objectNestLevel === 1)
                  {
                  // Note that we are going deeper into the object.
                  propertiesPath = objKey + ".properties";
                  }
                  else
                  {
                  // Note that we are going deeper into the object.
                  propertiesPath = propertiesPath + "." + objKey + ".properties";
                  }                  
                ...
                for (let property in address[objKey].properties)
                {
                  ...
                  { 
                    ...
                    // This replaces the underscore with dots to give us the objectName on the server from the variable name on the webpage.
                    let objectName =  objectPath.split('_').join('.');

                    if
                    (
                      // If there is a defaultElements object
                      address[objKey].properties[property].defaultElements 
                      // And it is populated with defaultElements
                      && Object.keys(address[objKey].properties[property].defaultElements).length > 0
                      // And one of the defaultElements has the same name as the field...
                      && address[objKey].properties[property].defaultElements.hasOwnProperty(property)
                    )
                    {
                      // Then we will have data coming from the form.
                      // So make javascript that takes data from the form and loads 
                      // it into the object we are going to write to the table
                      persistArray.push([objectName, objectPath, "bottomLevelDataField"])
                    }
                    else // There is no populated defaultElements property for this field.
                    {
                      // So make javascript that establishes the order of this field in the object but leaves the value undefined.
                      persistArray.push([objectName, "undefined", "bottomLevelDataField"])
                    }
          ...
          {  
            ...
            (
              // If there is a defaultElements object
              fieldsObject[fieldKey].defaultElements 
              // And it is populated with defaultElements
              && Object.keys(fieldsObject[fieldKey].defaultElements).length > 0
              // And one of the defaultElements has the same name as the field...
              && fieldsObject[fieldKey].defaultElements.hasOwnProperty(fieldKey)
            )
            {
              // then we will have data coming from the form.
              // So make javascript that takes data from the form and loads 
              // it into the object we are going to write to the table              
              persistArray.push([fieldKey, fieldKey, "topLevelDataField"])
            }
            else // There is no populated defaultElements property for this field.
            {
              // So make javascript that establishes the order of this field in the object but leaves the value undefined.
              persistArray.push([fieldKey, "undefined", "topLevelDataField"])
            }
          } // End of: else: this top level field is not an object
        } // End of: for (let fieldKey in fieldsObject){...}
        // End of: // Loop thorough the top level fields 

        // The above produces the persistArray which is populated as follows.
        // console.log(persistArray)


        let containerArray = [];
        let bottomLevelDatafieldArray = [];
        let lastWriteToBottomLevel = false;


        // This loop organizes information from the persistArray to be fed into the function writeOutContainer()
        persistArray.forEach(function(arrayElement)
        {
          if(arrayElement[2] === "topLevelDataField")
          {
            // Check if we have any bottomLevelDataFields to write out.
            if(bottomLevelDatafieldArray.length != 0)
            {
              writeOutContainer()
            }
            ...
            htmlString = htmlString +
            "      " + recordObject.table.tableName + "Object." + arrayElement[0] + " = " + arrayElement[1] + ";" + "\n";  
            
            lastWriteToBottomLevel = false;
          }
          else if(arrayElement[2] === "container")
          {
            if(lastWriteToBottomLevel)
            {
              writeOutContainer();
            }
            ...
            // Save information to write out
            containerArray.push(arrayElement)
          }
          else if(arrayElement[2] === "bottomLevelDataField")
          {
            // Save up information to write out.
            bottomLevelDatafieldArray.push(arrayElement)

            lastWriteToBottomLevel = true;
          }

        }); // End of persistArray.forEach(function(arrayElement){...}
        ...
        // Declare a function that writes javascript to persist nested documents.
        function writeOutContainer()
        {
          // Get the last part of the bottomLevelDatafieldArray[0].
          // That's everything after the last dot.
          // It's the fieldName without the object path.
          let lastPart = bottomLevelDatafieldArray[0][0].split(".")[bottomLevelDatafieldArray[0][0].split(".").length - 1]

          let firstZeroEncountered = true;

          // Write out the container objects.
          containerArray.forEach(function(element)
          {
            // if there is no zero in the element then it is a root container
            if(!element[0].includes("0"))
            {
              // Write it out first. These are not included in the for-let loop.
              htmlString = htmlString + 
`
      ${recordObject.table.tableName}Object.${element[0]} = {};`              
            }
            else // there is a zero in the element. This container will be included in the for loop we are generating.
            {
              if(firstZeroEncountered)
              {
                htmlString = htmlString +         
`
      for (let arrayIndex = 0; arrayIndex < ${lastPart}Array.length; arrayIndex++) 
      {`


                // Get the part of element[0] in front of the zero and the part behind it.
                let splitArray = element[0].split("0");

                // This is the splitArray with the container name chopped off.
                let containerPath = splitArray[0].split(".").slice(0, -1).join(".");

                // Just the container name without the zero.
                let containerName = splitArray[0].split(".")[splitArray[0].split(".").length -1]
              

                htmlString = htmlString + 
`
        ${recordObject.table.tableName}Object.${containerPath}["${containerName}" + arrayIndex.toString()] = {};
        ${recordObject.table.tableName}Object.${containerPath}["${containerName}" + arrayIndex.toString()].properties = {};` 


              } // End of: if(firstZeroEncountered){...}

              firstZeroEncountered = false;

            } // End of: Else: There is a zero in the Element. This container will be included in the for loop.
          }); // End of: containerArray.forEach(function(element){...}
          // End of: Write out the container objects.


          // Write out the container objects.
          bottomLevelDatafieldArray.forEach(function(element)
          {
            // Get the part of element[0] in front of the zero and the part behind it.
            let splitArray = element[0].split("0");
            ...
            let dataFieldName = splitArray[1].split(".")[splitArray[1].split(".").length -1]

            
            htmlString = htmlString + 
`
        ${recordObject.table.tableName}Object.${containerPath}["${containerName}" + arrayIndex.toString()].properties["${dataFieldName}"] = ${dataFieldName}Array[arrayIndex];`        

          }); // End of: bottomLevelDatafieldArray.forEach(function(element){...} 
...
`          
          ...
          // Reset containerArray and bottomLevelDatafieldArray for next encounter with a container.
          containerArray = [];
          bottomLevelDatafieldArray = [];
          lastWriteToBottomLevel = false;
        } // End of: function writeOutContainer(){...}
        // End of: Declare a function that writes javascript to persist nested documents.

        
        htmlString = htmlString +          
`      
      ${recordObject.table.tableName}Object.timeStamp = Date.now();
      ${recordObject.table.tableName}Object.deleted = false;
              ...
              { 
                ...
                for (let property in address[objKey].properties)
                {
                  ...
                  { 
                    ...
                    if(address[objKey].properties[property].calculation && Object.keys(address[objKey].properties[property].calculation).length > 0)
                    {
                      ...
                      for (let calcKey in address[objKey].properties[property].calculation)
                      {
                        if(calcKey === "postHandlerPreprocessing")
                        {
                          fieldCalculationCode = address[objKey].properties[property].calculation[calcKey]; 
                          break;           
                        }
                      } 
...
${fieldCalculationCode}
                ...
                `
                    ...
                    } // End of: if(address[objKey].properties[property].calculation && ...
                  } // End of: else - This property is not an object.
                } // End of: for-let Check each property of the subobject to see if it too is an object.
              } // End of: if the subfield is also an object
            } // End of: function drillIntoObject(address, objKey, previousObjKey, previousKeyPart, objectNestLevel){...}
        ...
        function(err)
        {
          ...
          function(err)
          {
            ...
            else // There was an error appending to ${recordObject.table.tableName}.
            {
              ...
              ${recordObject.table.tableName}Object.timeStamp = Date.now();
              ${recordObject.table.tableName}Object.deleted = true;
...
`  
        ...
        htmlString = htmlString +        
`
// ${recordObject.table.tableName} - put handler
// Define the ${recordObject.table.tableName} put subhandler function 
// This function updates a record.
// Required data: ${recordObject.table.tableName}Id
// Note: At least one other field must be specified.
// Behavior from meta.js at mzimrkdf1we1bjw96zgp
${recordObject.table.tableName}._${recordObject.table.tableName}.put = function(data, callback)
{
  // Field validation starts here.
  // Get ${recordObject.table.tableName}Id from payload
  let ${recordObject.table.tableName}Id = data.payload.${recordObject.table.tableName}Id;

  // PrimaryKey validation. 
  // Default behavior from meta.js at o65yzg6ddze2fkvcgw5s
  // If ${recordObject.table.tableName}Id is a valid string then convert it to a number.  
  if (typeof(${recordObject.table.tableName}Id) === 'string'){${recordObject.table.tableName}Id = parseInt(${recordObject.table.tableName}Id, 10);}else{return callback(400, {'Error' : '${recordObject.table.tableName}Id must be a of string type'});}
...
`
        ...
        for (let fieldKey in combinedDefaultElementsObject) 
        {
          ...
          for (let validationKey in mergedValidationObject)
          {
            ...
            if(validationKey === "passIfString")
            {
              // If the user supplied the validation string in the data dictionary
              if(mergedValidationObject[validationKey])
              {
                htmlString = htmlString + 
                "  // " + validationKey + "\n" +
...
`
              } // End of: Else no user supplied validation. Create a generic validation string.
            }
            else if(validationKey === "passIfNotEmpty")
            {
              // If the user supplied the validation string in the data dictionary
...
`
            ...
            else // Must be some other user defined code in the data dictionary.
            {
              // If the user supplied the validation string
              if(mergedValidationObject[validationKey])
              {
              ...
              { 
                ...
                for (let property in address[objKey].properties)
                {
                  ...
                  { 
                    ...
                    // If the current object is nested then prepend the path to include the parent objects.
                    objectPath = previousObjKey.length === 0 ? objectPath : previousObjKey + "_" + previousKeyPart + "_" + objectPath;
...
`  {
  ...
  _data.nextId(function(error, nextIdObject)
  {
    ...
    let ${recordObject.table.tableName}Object = {};
    ${recordObject.table.tableName}Object.${recordObject.table.tableName}Id = ${recordObject.table.tableName}Id;
...
`
        ...
        for (let fieldKey in fieldsObject)
        {
          ...
          if(fieldsObject[fieldKey].properties)
          {
            //Used for creating empty objects.
            propertiesPath = "    " + recordObject.table.tableName + "Object";
            ...
            { 
              ...
              if(address[objKey].properties)
              { 
                // If this is the first time we were this deep in the object on this branch then generate the following code.
                if(objectNestLevelHistory === objectNestLevel)
                {
                  htmlString = htmlString +
                  "\n" + propertiesPath + "." + objKey + " = {};" + "\n";
                ...
                for (let property in address[objKey].properties)
                {
                  ...
                  { 
                    ...
                    // This replaces the underscore with dots to give us the objectName on the server from the variable name on the webpage.
                    let objectName =  recordObject.table.tableName + "Object." + objectPath.split('_').join('.');
          ...
          {  
            ...
            )
            {
              ...
              // it into the object we are going to write to the table              
              htmlString = htmlString +
              "    " + recordObject.table.tableName + "Object." + fieldKey + " = " + fieldKey + ";" + "\n";
            }
            else // There is no populated defaultElements property for this field.
            {
              // So make javascript that establishes the order of this field in the object but leaves the value undefined.
              htmlString = htmlString +
              "    " + recordObject.table.tableName + "Object." + fieldKey + " = undefined;" + "\n";
            }  
...
`
        ...
        uniqueFieldsArray.forEach(function (elementValue, elementIndex)
        {
          ...
          "    dataObject.uniqueField0" + (elementIndex + 1) + "Value = " + recordObject.table.tableName + "Object." + elementValue + ";" + "\n";
        }); // End of: uniqueFieldsArray.forEach(function (elementValue, elementIndex)


        htmlString = htmlString +      
`    dataObject.path = '/${recordObject.table.directory}/${recordObject.table.tableName}.json';
    dataObject.queryString = 'WHERE:;${recordObject.table.tableName}Id:;MatchesExactly:;' + ${recordObject.table.tableName}Id + ':;';

    // This function returns the most recent record for this ${recordObject.table.tableName}Id after checking that 
    // data for unique fields is indeed unique and that the a record with the supplied ${recordObject.table.tableName}Id exists to modify.
...
`
                  ...
                  { 
                    ...
                    if(address[objKey].properties[property].calculation && Object.keys(address[objKey].properties[property].calculation).length > 0)
                    {
                      ...
                      else // No code in the data dictionary for this process. So build generic code.
                      {
                        htmlString = htmlString + 
                        "        // Preprocessing for " + objectPath + "\n" +
                        "        if(" + objectPath.split(".").join("_") + ") // If the user supplied data for " + objectPath + "\n" +
                        "        {" + "\n" +
                        "          // No preprocessing was specifed for " + objectPath + ". Use it as was supplied by the user." + "\n" +
                        "        }" + "\n" +
                        "        else // If the user did not supply data for " + objectPath + "\n" + 
                        "        {" + "\n" +
                        "          // Save " + objectPath + " from the most recent record." + "\n" +
                        "          " + recordObject.table.tableName + "Object." + objectPath + " = recordObject." + objectPath + ";" + "\n" +
                        "        }" + "\n\n";
          ...
          else // This top level field is not an object - It's just a regular data field.
          {
            if(fieldsObject[fieldKey].calculation && Object.keys(fieldsObject[fieldKey].calculation).length > 0)
            {
              ...
              else // No code in the data dictionary for this process. So build generic code.
              {
                ...
                "          " + recordObject.table.tableName + "Object." + fieldKey + " = recordObject." + fieldKey + ";" + "\n" +
                "        }" + "\n\n";
              } // End of: Else no code in the data dictionary for this process.              
            } // End of: if(fieldsObject[fieldKey].calculation &&...          
          } // End of: Else: this top level field is not an object...
        ...
        {    
          htmlString = htmlString +
          "          " + recordObject.table.tableName + "Object." + key + " = recordObject." + key + ";" + "\n";          
        }
   ...
   `
        ...
        {           
          ...
          "transactionId" : nextIdObject.nextId + 2,                 
          "rollback" : false,
          "process" : "${recordObject.table.tableName}._${recordObject.table.tableName}.put",
                ...
                function(err)
                {
                    ...
                    // start of callback code which is run after attempting to remove the lock.
                    {
                      ...
                      else // Good write but unable to remove lock on database.
                      {
                        ...
                        (
                          ...
                          '${helpers.createRandomString(20)}' + '\\n' +
                          'Successful write to ${recordObject.table.tableName} but unable to remove lock on database' + '\\n' +
                          'The following record was appended to ${recordObject.table.tableName}:' + '\\n' +                            
                          JSON.stringify(logObject) + '\\n' +   
                          'The following was the error message:' + '\\n' +                                             
                          error + '\\n'
                        ); // End of: helpers.log // Log the error.

                        callback(500, {'Error' : 'Successful write to ${recordObject.table.tableName} but unable to remove lock on database'});
                  ...
                  else // There was an error appending to ${recordObject.table.tableName}.
                  {
                    ...
                    (
                      ...
                      '${helpers.createRandomString(20)}' + '\\n' +
                      'There was an error when appending to the ${recordObject.table.tableName} file.' + '\\n' +
                      'The following record may or may not have been appended to ${recordObject.table.tableName}:' + '\\n' +                            
                      JSON.stringify(logObject) + '\\n' +
                      'Attempting to rollback the entry.' + '\\n' +    
                      'The following was the error message:' + '\\n' +                                             
                      err + '\\n'
                    ...
                    ${recordObject.table.tableName}Object = 
                    {
                        "${recordObject.table.tableName}Id" : recordObject.nextId,
`
...
`                        "timeStamp" : recordObject.timeStamp,
                    ...
                    callback(500, {'Error' : 'Could not create the new ${recordObject.table.tableName}.'});
...
}; // End of: handlers._${recordObject.table.tableName}.put = function(...
// End of: Define the ${recordObject.table.tableName} put subhandler function
...
`
// Define the ${recordObject.table.tableName} get subhandler function.
// Streams the ${recordObject.table.tableName} file or part of it back to the client.
${recordObject.table.tableName}._${recordObject.table.tableName}.get = function(data, callback)
{
  ...
  if (data.hasOwnProperty('queryString') || typeof(Object.keys(data.queryStringObject)[0]) != 'undefined')
  {
    ...
    {
      // In this case the queryString is coming from ${recordObject.table.tableName}Edit page.
      queryString = data.queryString
    }
    else
    {
      // In this case the queryString is coming from the ${recordObject.table.tableName}List page.
  ...
  // Create an empty map data structure which will be used to merge ${recordObject.table.tableName} records that have the same unique fields.
  // Chose map data structure over objects because maps are guaranteed to maintain the same order where as objects are not.
  let ${recordObject.table.tableName}Map = new Map();
  ...
  readInterface.on('line', function(line) 
  {
    // Convert the JSON string (a single line from the ${recordObject.table.tableName} file) into lineValueObject.
    ...
    let ${recordObject.table.tableName}Id = lineValueObject.${recordObject.table.tableName}Id;      

    if(lineValueObject.deleted === true) // if the record in the file ${recordObject.table.tableName}.json had the delete field set to true:
    {
      // Remove this record from the map 
      ${recordObject.table.tableName}Map.delete(${recordObject.table.tableName}Id);
              ...
              { 
                ...
                for (let property in address[objKey].properties)
                {
                  ...
                  { 
                    ...
                    if(address[objKey].properties[property].published === false)
                    {
...
`      // Remove the deleted key/value pair from the lineValueObject before returning it to the requester.
      ...
      ${recordObject.table.tableName}Map.set(${recordObject.table.tableName}Id, lineValueObject);
    }
  ...
  // This listener fires after we have looked through all the records in the ${recordObject.table.tableName} file.
  // The callback function defined here will stream the ${recordObject.table.tableName} list back to the clients browser.
  readInterface.on('close', function() 
  {          
    // This readable stream will be used to write the result of the merge to a new file.
    const sourceStream = new Readable(); 

    for (const [key, valueObject] of ${recordObject.table.tableName}Map)
    {
...
}; // End of: handlers._${recordObject.table.tableName}.get = function(data, callback){do stuff}
// End of: Define the ${recordObject.table.tableName} get subhandler function.  




// Export the module
module.exports = ${recordObject.table.tableName};
        ...
        (
          ...
          function(err)
          {
            if (!err) // The javascript string was written successfully to a file.
            {
              ...
              'The javascript file ' + recordObject.table.tableName + "Logic" + '.js was written successfully to a file.' + '\n'                                  
              ); // End of: helpers.log(...)
            }
  ...
  helpers.getMostRecent(dataObject, function(errorFromGetMostRecent, payload)
  {
    if(!errorFromGetMostRecent) // Got the most recent record from getMostRecent
    {
      ...
      writable.write = function(payload)     
      {
        ...
        "    <a class=\"cta green\" href=\"" +recordObject.table.tableName + "/add\">Create New</a>" + "\n" +
        "  </div>" + "\n" +
        "</div>" + 
        "\n" + "\n" +
        "<hr />" + 
          ...
          {    
            ...
            if(fieldsObject[fieldKey].properties)
            {
              ...
              function drillIntoObject(obj, objKey, startingPath)
              {
                ...
                // Here is where we do the drilling.
                for (let keyPart in objValue[objKey].properties) 
                {
                  let propertyPath = startingPath + objKey + ".properties." + keyPart;

                  // if the subfield is also an object then this function calls itself using the subfield parameters.
                  // This will continue drilling in until we are at the end of the branches and
                  if(objValue[objKey].properties[keyPart].properties)
                  { 
                    drillIntoObject(objValue[objKey].properties, keyPart, objKey + ".");
                  }
                  else // We are at the end of a branch. This is a field not an object so create HTML for this field.
                  {
                    if(objValue[objKey].properties[keyPart].published === true)
                    {
...
app.load${recordObject.table.tableName}EditPage = async function()
{
  ...
  // Get ${recordObject.table.tableName}Id
  ...
  // Create a queryExpression with a key of ${recordObject.table.tableName}Id and a value of the 
  ...
  // This keeps class and element names of container objects the same.
  // The querySelector used while data is loading requires this.
...
`
        ...
        // Need an array with the path to all object containers.
        ...
        for (let fieldKey in fieldsObject)
        {
          ...
          if(fieldsObject[fieldKey].properties)
          {
              ...
              { 
                ...
                for (let property in address[objKey].properties)
                {
                  ...
                  { 
                    ...
                    if(address[objKey].properties[property].published === true)
                    {                      
                      containersArray.push([previousPath, objKey])
                    }
            ...
            // This function will call itself several times if drilling into nested objects is required.
            drillIntoObject(fieldsObject, fieldKey, objectNestLevel, "")
          } // End of: If this top level field is an object containing more fields inside...
        } // End of: for (let fieldKey in fieldsObject){...}
        // End of: // Loop thorough the top level fields  


        // Don't want to use the set data structure to enforce uniqueness 
        // until I understand it's effect on garbage collection.
        // Using this function from @Stephen on stackexchange instead. Thanks
        ...
        let uniqueContainers = multiDimensionalUnique(containersArray); 

        
      

        uniqueContainers.forEach(function(element, elementIndex)
        {
          let containerNoNumber = element[1].slice(0, -1)

          let fieldPath = element[0].slice(0, -1).split("_").join(".");

          htmlString = htmlString +
`
  // Get the ${containerNoNumber} count.
  const ${containerNoNumber}Amount = Object.keys(recordObject.${fieldPath}).length;

  // Clone enough elementWrappers to accommodate every ${containerNoNumber}.
  for (let loopCount = 0; loopCount < ${containerNoNumber}Amount - 1; loopCount++) 
  {
    const addButton = document.querySelectorAll('.addNewObjectButton')[${elementIndex}];
    addButton.click();
  };
`                    
              ...
              { 
                ...
                for (let property in address[objKey].properties)
                {
                  ...
                  { 
                    ...
                    if(address[objKey].properties[property].published === true)
                    {
                      let previousPathWithDots = previousPath.slice(0, -1).split("_").join(".");

                      let containerNoNumber = objectPath.split("0")[0].split("_")[objectPath.split("0")[0].split("_").length - 1];

                      let fieldPath = objectPath.split(0)[1].slice(1).split("_").join(".");

                      htmlString = htmlString +
`
  document.querySelectorAll(".${objectPath}Input").forEach
  (
    function (element, elementIndex)
    {
      element.value = recordObject.${previousPathWithDots}["${containerNoNumber}" + elementIndex.toString()].${fieldPath};
    }
  );
`
                    } // End of: if(address[objKey].properties[property].published === true){...}
                  } // End of: else - This property is not an object.
                } // End of: for-let Check each property of the subobject to see if it too is an object.
              } // End of: if the subfield is also an object
            }; // End of: function drillIntoObject(address, objKey, objectNestLevel, previousPath){...}
          ...
          else // This top level field is not an object - It's just a regular field.
          {
            ...
            {
              htmlString = htmlString +
              "  document.querySelectorAll(\"." + fieldKey + "Input\").forEach(function (element){element.value = recordObject." + fieldKey + "});" + "\n";
            }           
          }
...
meta.makeAddPageElementHtml = function(elementName, recordObject)
{
  ...
  if(recordObject.webPages.addPage.forms.addForm.formElements[elementName].properties)
  {
    ...
    // Declare a function that we will use to drill into the object and make HTML as we travers the branches.
    function drillIntoObject(formElementObj, objKey, fieldsObject, objectNestLevel, previousPath)
    {      
      // if the subfield is also an object then this function calls itself using the subfield parameters.
      // This will continue drilling into the object until we are at the end of the branches.
      if(formElementObj[objKey].properties)
      { 
        // Check each property of the subobject to see if it too is an object.
        for (let property in formElementObj[objKey].properties)
        {
          // If the property is also an object.
          // We can tell because objects in this system have a property called "properties"
          if(formElementObj[objKey].properties[property].properties)
          {

            // Commented out because a label is not needed here and because the label interferes with 
            // the operation of the up and down controls.
            // elementString = elementString + "  ".repeat(objectNestLevel) +
            // "    <div class=\"objectLabel\">" + formElementObj[objKey].properties[property].labelText + "</div>" + "\n"; 

            // Make the HTML for a container to hold this object's properties.
            ...
            // This function calls itself so as to go deeper into the object
            drillIntoObject(formElementObj[objKey].properties, property, fieldsObject[objKey]["properties"], objectNestLevel + 1, previousPath + objKey + "_properties_");
          ...
          else // This property is not an object. We are at the end of the branch where we encounter a formElement.
          { 
            ...
            let defaultElementsObject = fieldsObject[objKey].properties[property].defaultElements[property];

            // console.log("11. This is the defaultElementObject\n", defaultElementsObject, "\n\n");
          
            // Deep merge the defaultElements with formElements - formElements taking precedence.
            let mergedElements = extend(true, defaultElementsObject, formElementObj[objKey].properties[property]);
...
meta.makeEditPageElementHtml = function(key, recordObject)
{
  ...
  {
   // If we encounter a formElement which is an object then make an HTML container and create HTML for the sub elements.
    if(recordObject.webPages.editPage.forms[key].formElements[elementValue].properties)
        {
      // Add a label to the object container if one was provided.
      ...
      {      
        ...
        { 
          ...
          {
            // If the property is also an object...
            // We will know because objects contain a property called "properties"
            if(formElementObj[objKey].properties[property].properties)
            {
              // Make the HTML for a container to hold this object's properties.
              // objectNestLevel handles indentation as we make HTML for the addPage.
              ...
              // Commented out because a label is not needed here and because the label interferes with 
              // the operation of the up and down controls.
              // "  ".repeat(objectNestLevel) + "    <div class=\"objectLabel\">" + formElementObj[objKey].properties[property].labelText + "</div>" + "\n" + 
