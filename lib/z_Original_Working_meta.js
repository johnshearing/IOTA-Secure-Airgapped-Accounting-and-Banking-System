    ...
    if(recordWasDeleted === false)
    {
      `
        ...
        for (let fieldKey in fieldsObject)
        {
              ...
              { 
                ...
                for (let property in address[objKey][objKey + "0"])
                {
                  ...
                  { 
                    ...
                    if(address[objKey][objKey + "0"][property].published === "no")
                    {
          ...
          {
            if(fieldsObject[fieldKey].published === "no")
            {
    ...
    if(!errorFromGetMostRecent) // Got the most recent record from getMostRecent
    {
      ...
      writable.write = function(payload)     
      {
        ...
        keysOfFormElementsArray.forEach(function (elementValue)
        {
          ...
          let resultArray = meta.makeAddPageElementHtml(elementValue, recordObject, madeHideShowControl)

          htmlString = htmlString + resultArray[0];

          if(resultArray[1] === true){madeHideShowControl = true;}
        ...
        // Build HTML for any datalists
        let datalistsObject = recordObject.datalists;

        // Loop through the datalistsObject.
        for (let listsKey in datalistsObject)
        {

          htmlString = htmlString +
          "<datalist id=\"" + listsKey +"\">" + "\n"

          for (let optionKey in datalistsObject[listsKey])
          {
            htmlString = htmlString +            
        ...
        } // End of: for (let listsKey in datalistsObject)
        // End of: // Loop through the datalistsObject.
...
function renumberNamedElementsInGroupWrapper(groupWrapper)
{
  ...
  groupWrapper.querySelectorAll('.elementWrapper').forEach(function(elementWrapper, wrapperIndex)
  {
    ...
    elementWrapper.querySelectorAll('.resetElement').forEach(function(fieldElement)
    {
      ...
      //Debug let elementColor = fieldElement.style.backgroundColor;
      //Debug fieldElement.style.backgroundColor  = "blue";
    ...
    if(!errorFromGetMostRecent) // Got the most recent record from getMostRecent
    {
      ...
      writable.write = function(payload)     
      {
        ...
        for (let fieldKey in fieldsObject) 
        {
          ...
          if(fieldsObject[fieldKey].published === "yes")
          {    
            ...
            if(fieldsObject[fieldKey][fieldKey + "0"])
            {
              ...
              function drillIntoObject(obj, objKey, startingPath)
              {
                ...
                // Here is where we do the drilling.
                for (let keyPart in objValue[objKey][objKey + "0"]) 
                {
                  let propertyPath = startingPath + objKey + objKey + "0" + keyPart;

                  // if the subfield is also an object then this function calls itself using the subfield parameters.
                  // This will continue drilling in until we are at the end of the branches and
                  if(objValue[objKey][objKey + "0"][keyPart][keyPart + "0"])
                  { 
                    drillIntoObject(objValue[objKey][objKey + "0"], keyPart, objKey + ".");
                  }
                  else // We are at the end of a branch. This is a field not an object so create HTML for this field.
                  {
                    if(objValue[objKey][objKey + "0"][keyPart].published === "yes")
                    {
        ...
        // End of: // Loop through the fieldsObject.
      ...
      writable.write = function(payload)     
      {
        ...
        for (let key in editPageFormsObject) 
        {
          ...
          if(key === "editForm")
          {
            ...
            // The following inserts the expand/collapse controls over the first occurance of nested data.
            // Search htmlString for <div class="objectLabel">
            // If we find it then insert HTML for the controls.

            htmlString = htmlString.replace
            (
              "    <div class=\"objectLabel\">", // This is the search string. The next line is the replace string.
...
// Behavior from meta.js at 6z4zby99ilbammzmzy8f
app.load${recordObject.tableName}EditPage = async function()
{
  ...
  function onClickBehaviorForAddNewObjectButton (event)  
  {
    ...
    // Match range lists with domain elements.
    cln.querySelectorAll("input").forEach(function(currentInput)
    {
      if(currentInput.getAttribute("rangeFor"))
      {
        let rangeIdentifier = currentInput.getAttribute("rangeFor");

        document.querySelectorAll("input, select").forEach(function(domainCandidate)
        {
          if(domainCandidate.getAttribute("domainFor") === rangeIdentifier)
          {
            currentInput.setAttribute("list", domainCandidate.value);
          }          
        });          
      }
    });
    // End of: Match range lists with domain elements.
...
meta.makeAddPageElementHtml = function(elementName, recordObject, madeHideShowControl)
{
  ...
  if(recordObject.webPages.addPage.forms.addForm.formElements[elementName][elementName + "0"])
  {
    ...
    {
      // Make HTML for the expand/collapse control
      if(madeHideShowControl === false)
      {
        elementString = elementString + 
        "    <span class=\"expandControl\" style=\"color:blue;font-weight:bold\">&#10515;&nbsp;&nbsp;</span><span class=\"collapseControl\" style=\"color:blue;font-weight:bold\">&#10514;</span>" + "\n";

        // Ensures the controls are made only once.
        madeHideShowControl = true;
      }
      ...
      for (let property in formElementObj[objKey][objKey + "0"])
      {
        ...
        if (formElementObj[objKey][objKey + "0"][property][property + "0"])
        {
          ...
          if(formElementObj[objKey][objKey + "0"])
          {
            ...
            "      <button type=\"button\" class=\"addNewObjectButton\"" +
            " style=\"background-color: " + 
            borderColor + ";\"" +              
            ">Add New " + property[0].toUpperCase() + property.slice(1) + "</button>"  + "\n"; 
        ...
        { 
          ...
          elementString = elementString +
          "    ".repeat(objectNestLevel) + "    <div class=\"inputWrapper\">" + "\n";
          ...
          if(mergedElements.labelText)
          {
            ...
            elementString = elementString +
            "    ".repeat(objectNestLevel) + "      <div class=\"inputLabel\">" + mergedElements.labelText + "</div>" + "\n";        
          }
          else // We did not specify labelText. Make a generic label programmatically.
          {
            elementString = elementString +
            "    ".repeat(objectNestLevel) + "      <div class=\"inputLabel\">" + elementNameInTitleCase + "</div>" + "\n";
          } // End of: If we have specifed...
    ...
    " style=\"background-color: " + 
    "blue" + ";\"" + 
    ">Add New " + elementName[0].toUpperCase() + elementName.slice(1) + "</button>"  + "\n";    
    ...
    return [elementString, madeHideShowControl];  
...
meta.makeEditPageElementHtml = function(key, recordObject)
{
  ...
  keysOfFormElementsArray.forEach(function (elementValue)
  {
    ...
    if(recordObject.webPages.editPage.forms[key].formElements[elementValue][elementValue + 0])
      ...
      {    
        ...
        for (let property in formElementObj[objKey][objKey + "0"])
        {
          ...
          if (formElementObj[objKey][objKey + "0"][property][property + "0"])
          {
            ...
            // Border color alternates with nest level. Blue, Red, Green, Blue
            // The following is just an if-else statement. 
            let borderColor =  objectNestLevel % 3 === 0 ? "blue" : objectNestLevel % 2 === 0 ? "green" : "red"
      ...
      " style=\"background-color: " + 
      "blue" + ";\"" + 
      ">Add New " + elementValue[0].toUpperCase() + elementValue.slice(1) + "</button>"  + "\n";    
