...
const aHandlers = require('./aHandlers');
const aHelpers = require('./aHelpers');
const user = require('../accounting/database/dbPermission/user/userLogic');
const oPhoneType = require('../accounting/ourSelf/oLookup/oPhoneType/oPhoneTypeLogic');
const oContactType = require('../accounting/ourSelf/oLookup/oContactType/oContactTypeLogic');
const oAddressType = require('../accounting/ourSelf/oLookup/oAddressType/oAddressTypeLogic');
const oContact = require('../accounting/ourSelf/oContact/oContactLogic');
const metadata = require('../accounting/database/dbMetadata/metadataLogic');
...
server.unifiedServer = function(req, res)
{
  ...
  req.on('end', function()
  {
    ...
    chosenHandler = trimmedPath.indexOf('public/') > -1 ? aHandlers.public : chosenHandler;

    // If the request is within the accounting directory, use the accounting handler instead of what was assigned to
    // the chosen handler originally two lines of code above.
    // The line below is required because the line above will only match a handler to a client request if
    //   the request exactly matches one of the keys in the request router object at the bottom of this file. 
    //   So if the request is "accounting" then we have a match with the accounting handler. But request 
    //   accounting/.../app.js would not be matched with a handler.
    chosenHandler = trimmedPath.indexOf('accounting/') > -1 ? aHandlers.accounting : chosenHandler;    
...
server.router = {
  ...
  'favicon.ico' : aHandlers.favicon,
  'public' : aHandlers.public,
  'accounting' : aHandlers.accounting,  
  'user/add' : user.serveAddPage,
  'user/list' : user.serveListPage,  
  'user/edit' : user.serveEditPage,
  'api/user' : user.user,  
  'oPhoneType/add' :oPhoneType.serveAddPage,
  'oPhoneType/list' :oPhoneType.serveListPage,
  'oPhoneType/edit' : oPhoneType.serveEditPage,
  'api/oPhoneType' : oPhoneType.oPhoneType,
  'oContactType/add' :oContactType.serveAddPage,
  'oContactType/list' :oContactType.serveListPage,
  'oContactType/edit' : oContactType.serveEditPage,
  'api/oContactType' : oContactType.oContactType,  
  'oAddressType/add' :oAddressType.serveAddPage,
  'oAddressType/list' :oAddressType.serveListPage,
  'oAddressType/edit' : oAddressType.serveEditPage,
  'api/oAddressType' : oAddressType.oAddressType,
  'oContact/add' :oContact.serveAddPage,
  'oContact/list' :oContact.serveListPage,
  'oContact/edit' : oContact.serveEditPage,
  'api/oContact' : oContact.oContact, 
  'metadata/add' : metadata.serveAddPage,
  'metadata/list' : metadata.serveListPage,  
  'metadata/edit' : metadata.serveEditPage,
