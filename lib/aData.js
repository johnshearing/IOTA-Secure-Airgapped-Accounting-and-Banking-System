...
lib.gsuidLockReadIncUnlock = function(idRange, callback)
{
  ...
  fs.mkdir(dbHistoryDir + 'gsuid.lock', function(error)
  {
    if(!error) // the file was created successfully.
    {
      ...
      lib.read('dbHistory', 'gsuid', function(error, nextIdObject)
      {
        ...
        if(!error && nextIdObject)
        {
          ...
          lib.update('dbHistory', 'gsuid', nextIdObject, function(error)
          {
...
lib.log = function(logLevel, message) 
{
  if (logLevel >= config.logLevel)
  {
    ...
    { 
      ...
      let wLogStream = fs.createWriteStream(lib.baseDir + 'dbLogs' + '/' + 'activityLog' + '.log', {flags : 'a'});
