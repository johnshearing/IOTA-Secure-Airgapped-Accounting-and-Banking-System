...
lib.gsuidLockReadIncUnlock = function(idRange, callback)
{
  ...
  try 
  {
    ...
    lib.read('dbHistory', 'gsuid', function(err, nextIdObject)
    {
      ...
      if(!err && nextIdObject)
      {
        ...
        lib.update('dbHistory', 'gsuid', nextIdObject, function(err)
        {
          if(!err) // There was no error writing the incremented nextId back to the file.
          {
            ...
            lib.log('qtjwjum5b9l4ww8wo1rb');
            lib.log('Got a good locked read');
            lib.log(' ');
          ...
          else // There was an error writing the incremented nextId back to the file.
          {
            ...
            lib.log('97xxiel6uubzxgzc1j14');
            lib.log('There was an error writing the incremented nextId back to the file:');
            lib.log('This was the error:');
            lib.log(err);            
            lib.log(' ');
      ...
      else // Problem reading gsuid.json. There was an error or no data was returned.
      {
        ...
        lib.log('vqihr0azmm9vasc21885');
        lib.log('Problem reading gsuid.json.');
        lib.log('This is the error:');
        lib.log(err);
        lib.log(' ');
  ...
  catch(err) 
  {
    ...
    {   
      lib.log('zz1iv0q1o0702wk5e3so')
      lib.log('Could not get a lock.');    
      lib.log(' ');
    ...
    else // The file was not already locked but there was another problem.
    {
      ...
      lib.log('clp1z82jq8wauo0y7fbc');
      lib.log('An error occured when trying to get a locked read on the Global Sequential Unique ID');
      lib.log('This was the error:');
      lib.log(err); 
      lib.log(' ');
...
lib.nextId = function(idRange, callback)
{
  ...
  {
    lib.log('r2rffhtkfpobma6qsj5l');
    lib.log('The tryCount is ' + tryCount);
    lib.log(' ');
    ...
    if(stopTrying == false && nextIdObject == false)
    {
      ...
      setInterval(lib.gsuidLockReadIncUnlock, 3000, idRange, function(stopTrying, error, nextIdObject)
      {
        ...
        if (stopTrying == false && nextIdObject == false)
        {
          ...
          lib.log('gnx7bp81kmswpajdo06g');
          lib.log('The try count is ' + tryCount);
          lib.log(' '); 
        }
        ...
        else if(stopTrying == true && nextIdObject == false) 
        {
          ...
          lib.log('p9hjguitshfc7vxxxs93');
          lib.log('We got a lock on the gsuid file but there was a problem reading it.');
          lib.log('This was the error:');
          lib.log(error);    
          lib.log(' ');        
             ...
             // to increment the id and write it back to the file.
        {
          ...
          lib.log('kv323g8n4otmv5m08zfw');
          lib.log('Success! The next Id is the following:');
          lib.log(nextIdObject);
          lib.log(' ');
        ...
        if(tryCount == 5)
        {
          ...
          lib.log('1xjvjk25hsx1z81ov41b');
          lib.log('Maximum number of tries has been exceeded.');
          lib.log(' ');
    ...
    {
      lib.log('egnqfq48ru4319c7sa4b');
      lib.log('We got a lock on the gsuid file but there was a problem reading or writing to it.');
      lib.log('This was the error:');
      lib.log(error);   
      lib.log(' ');
...
lib.append = function(dir, fileName, appendObject, callback)
{
  ...
  var stringData = JSON.stringify(appendObject);

  // Open a writable stream.
  let wStream = fs.createWriteStream(lib.baseDir + dir + '/' + fileName + '.json', {flags : 'a'});

  // log an error if we can not create a write stream.
  wStream.on('error', function(error)
  {
    lib.log('pmazz7shsko8mfnc0gyz');
    lib.log('Error creating a writable stream for ' + fileName);
    lib.log('This was the error:');
    lib.log(error);   
    lib.log(' ');
  });
  ...
  wStream.on('open', function() 
  {
    ...
    wStream.write(stringData + '\n', function(error)
    {
      ...
      {
        lib.log('h0fceuftq8xkdkvh4dl9');
        lib.log('Error appending to file ' + fileName);
        lib.log('This was the error:');
        lib.log(error);   
        lib.log(' ');
...
lib.read = function(dir, file, callback)
{
  ...
  fs.readFile(lib.baseDir + dir + '/' + file + '.json', 'utf8', function(err, data)
  {
    ...
    {
      lib.log('qs9tayvevafco6598owy');
      lib.log('Could not read file ');
      lib.log('This was the error:');
      lib.log(err);   
      lib.log(' ');
  ...
  fs.open(lib.baseDir + dir + '/' + file + '.json', 'r+', function(err, fileDescriptor)
  {
    ...
    if(!err && fileDescriptor) 
    {
      ...
      fs.ftruncate(fileDescriptor, function(err)
      {
          ...
          fs.writeFile(fileDescriptor, stringData, function(err)
          {
            if(!err)
            {
              fs.close(fileDescriptor, function(err)
              {
                ...
                {
                  lib.log('ans83rc3oh0y2z86dxu7');
                  lib.log('Could not close file ');
                  lib.log('This was the error:');
                  lib.log(err);   
                  lib.log(' ');
            ...
            {
              lib.log('sq5f7qfn87t98o6ix2pu');
              lib.log('Error writing to existing file.');
              lib.log('This was the error:');
              lib.log(err);   
              lib.log(' ');
        ...
        {
          lib.log('1hg33z6svppig8tnf4eo');
          lib.log('Error truncating the file');
          lib.log('This was the error:');
          lib.log(err);   
          lib.log(' ');
    ...
    {
      lib.log('36samm83kf5igbu0y0gt');
      lib.log('Could not open file for updating. It may not exist yet');
      lib.log('This was the error:');
      lib.log(err);   
      lib.log(' ');      
