...
handlers._users.put = function(data, callback){
  ...
  if(userId && (email || password))
  {
    ...
    _data.nextId(idRange, function(error, nextIdObject)
    {
      ...
      if(!error && nextIdObject)
      {
        let hashedPassword; // Declared here so we can see in in all the subfunctions.
        ...
        } // End of: If the user supplied a password and it passed validation.        
        ...
        else // No new password. We need to lookup the hashed password from the previous record.
        {
          ...
          helpers.getHashedPass(dataObject, function(errorFromGetHashPass, payload)
          {
            if(!errorFromGetHashPass) // Got the most recent record from gitHashedPass
            {
            ...
            else // There was indeed an error returned by getHashedPass when attempting to get the most current record.
            {
              ...
              // start of callback code which is run after attempting to remove the lock.
              {
                ...
                {
                  if(errorFromGetHashPass === "No record found for userId")
                  {
                    helpers.log // Log the error.
                    (
                      7,
                      'nx0mklkr92hedunnegto' + '\n' + 
                      'No record found for userId' + '\n'                                                 
                    ); // End of: helpers.log // Log the error.
  
                    callback(500, {'Error' : 'No record found for userId'});      
                  }
                }
                else // Error from getHashedPass and also unable to remove lock on database.
                {
                  if(errorFromGetHashPass === "No record found for userId")
                  {
                    helpers.log // Log the error.
                    (
                      7,
                      'ng0mkbkr92hequnnegpo' + '\n' +
                      'No record found for userId and also unable to remove lock on database.' + '\n' +       
                      'The following was the error message from removeLock:' + '\n' +                                             
                      errorFromRemoveLock + '\n'
                    ); // End of: helpers.log // Log the error.
  
                    callback(500, {'Error' : 'No record found for userId and unable to remove lock on database'});      
                  }
                } // End of: else Good write but unable to remove lock on database.
          ...
          }); //End of: helpers.getHashedPass(dataObject, function(error, payload)
