...
{ 
  ...
  readInterface.on('line', function(line) 
  {
    ...
    else if(amountOfWhereClauses > 0) // else if the user created one or more filter expressions
    {
      ...
      let indexOfNextPossibleOrWhere // Will set a value later. Just want to make sure we can see it outside the while loop.
      ...
      while(whereClauseCount <= amountOfWhereClauses && shouldLoopAgain === true)
      {
        ...
        // All (ANDWHERE ORWHERE) clauses will be evaluated from left to right with no preference to the type of conjunction.
        // This should allow any type of expression to be evaluated without the need for parentheses.
        ...
        {      
          indexOfNextPossibleOrWhere = index + 3;
          ...
          // Deleting record and dropping out of both while loops. 
        ...
        // On the other hand - if the record passed through the current filter then we need to 
        // see if there is an ANDWHERE clause right next to the current filter. 
        // Any further ANDWHERE filters right next to this current one must also pass the record
        // until there are no more filters or an ORWHERE filter is found. 
        else // shouldDeleteThisRecord was false
        {
          // PASSING THIS RECORD FOR NOW!!!
          // Just for now we are passing this record and dropping out of 
          // both while loops without checking for more ANDWHERE clauses.
          shouldLoopAgain = false; 
          recordWasDeleted = false;  
        }      
