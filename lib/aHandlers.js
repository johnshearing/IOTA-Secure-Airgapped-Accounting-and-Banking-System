...
handlers.dbUsersAdd = function(data, callback)
{
  // Reject any request that isn't a get
  if(data.method == 'get')
  {
    ...
    var templateData = 
    {
      ...
      'body.class' : 'dbUsersAdd',      
      'head.clientCode' :  'accounting/dbPermissions/dbUsers' + '/' + 'dbUsersAdd' + '.js',      
    };

    // Read in a template as a string
    helpers.getTemplate('../accounting/dbPermissions/dbUsers/dbUsersAdd', templateData, function(errorGetTemplate, str)
    {
      if(!errorGetTemplate && str) // If there were no errors and a template was returned
      {
        // Add the universal header and footer.
...
}; // End of: handlers.dbUsersAdd = function(data, callback){...}
...
* the request is received by req.on('end'... which routes the request to the handlers._dbUsers.post function
...
handlers.dbUsers = function(data, callback)
{
  ...
  if (acceptableMethods.indexOf(data.method) > -1) 
  {
    ...
    handlers._dbUsers[data.method](data, callback);
  } 
          ...
          _data.nextId(idRange, function(error, nextIdObject)
          {
            ...
            if(!error && nextIdObject)
            {
              ...
              var logObject =
              {
                ...
                "rollback" : false,
                "process" : "aHandlers._dbUsers.post",
...
}; // End of: handlers._dbUsers.post = function(...
...
}; // End of: handlers._dbUsers.get = function(data, callback){do stuff}
...
handlers._dbUsers.put = function(data, callback){
  ...
  if(userId && (email || password || deleted))
  {
    ...
    _data.nextId(idRange, function(error, nextIdObject)
    {
      ...
      if(!error && nextIdObject)
      {
        ...
        dataObject.path = '/dbPermissions/dbUsers/dbUsers.json';
          ...
          if(!errorFromGetMostRecent) // Got the most recent record from getMostRecent
          {
            ...
            writable.write = function(payload)
            {
              ...
              var logObject =
              {
                ...
                "logId" : nextIdObject.nextId + 2,    
                "rollback" : false,
                "process" : "aHandlers._dbUsers.put",
...
}; // End of: handlers._dbUsers.put = function(...
