...
handlers._users.post = function(data, callback)
{
  ...
  if(email && password)
  {
    ...
    // Get a lock on the dbLog file.
    // This makes the system multiuser.
    // All writes to any table must first get a lock on dbLog.json
    // dbLog.json stays locked until the operation is completely finished.
    // This ensures that only one process is writing to the database at any one time.
    /*
    _data.dbLock
    (
      function // This is the callback to perform after _data.dbLock runs.
      (
        error // This value is called back from _data.dbLock to let us know if we got a lock or not.
      )
      {
        if(!error) // We have a lock on dbLog.json so do the following
        {

        } // End of: We have a lock on dbLog.json so do the following
        else // We did not get a lock on dbLog so do not attempt to write anything to the database.
        {

        } // End of: We did not get a lock on dbLog so do not attempt to write anything to the database.
      } // End of: This is the callback to perform after _data.dbLock runs.
    ); // End of: _data.dbLock
    // End of: Get a lock on the dbLog file.
    */
    ...
    readInterface.on('line', function(line) 
    {
      ...
      if (email == lineObject.email && lineObject.deleted == false) // Not unique.
      {
        emailIsUnique = false; // This flag used in the on close event listener below. 
      }
      else // The record is unique.
      {
        emailIsUnique = true;
      }
    ...
    readInterface.on('close', function() 
    {
      if (emailIsUnique) // The email address is unique so proceed with append operation.
      {
        ...
        if(hashedPassword) // The hash function successfully returned the hashed password.
        {
          ...
          _data.nextId(idRange, function(err, nextIdObject)
          {
                  ...
                  if (!err)  //The dbLog file has been appended to successfully.
                  {
                    ...
                    (
                      ...
                      function(err)
                      {
                        ...
                        {
                          _data.removeLock(function(error)
                          {
    ...
    }); // End of: readInterface.on('close', function(line)
