...
handlers._users.put = function(data, callback){
  ...
  /*
  // Check the required userId is of type number. 
  // If not then set to boolean false.
  var userId = typeof(data.payload.userId) == 'number' ? data.payload.userId : false;
  ...
  if(userId) // If the phone number passed validation test.
  {
    ...
    if(firstName || lastName || email || password)
    {
      ...
      // Maybe add a new record with the updated information.
  */
  ...
  {
    if(password) // if the user supplied a password and it passed validation.
    {
      // Hash the password
      var hashedPassword = helpers.hash(password);
    ...
    let userIdIsUnique = true; // Will toggle this to false if we find the email address already exists in dbUsers.

    // To ensure the email address exists already we will read every record in 
    // dbUsers and compare with the email address provided.
    ...
    // Look at each record in the file and set a flag if the userID matches the userId provided by the user.
    readInterface.on('line', function(line) 
    {
      // Convert the JSON string from dbUsers into an object.
      lineObject = JSON.parse(line);
      ...
      if (userId == lineObject.userId) // we found a matching entry
      {
        if (lineObject.deleted == false) // The record has not been deleted so it's a duplicate. Not unique.
        {
          userIdIsUnique = false; // This flag used in the on close event listener below. 
        }
        // The matching record we found has been deleted so it may as well not exist. The new record is still unique.
        else 
        {
    ...
    readInterface.on('close', function() 
    { 
      // Check if there is a record to change.
      if (!userIdIsUnique) // The userId is NOT unique (there is a record to change). So proceed with append operation.
      {
        // Specify the amount of unique IDs needed.
        ...
        _data.nextId(idRange, function(error, nextIdObject)
        {
          ...
          if(!error && nextIdObject)
          {
            ...
            // If the user did not supply a new password to hash then we need 
            // to look up the most recent hashed password from the previous record.
            if(password) // The user supplied a new password.
            {
              // The user gave us a new password to hash.
              userObject.hashedPassword = hashedPassword
            }
            else // We need to lookup the hashed password from the previous record.
            {
              //???
              dataObject = {};
              ...
              // handlers._users.get(dataObject, function(statusCode, payload, contentType)
              helpers.getHashedPass(dataObject, function(statusCode, payload, contentType)
              {
                ...
                writable.write = function(payload)
                {
                  ...
                  console.log('This is the recordObject:' + '\n', recordObject)
                };
                ...
                /*
                writable._write = (string, encoding, done) => 
                {
                  let stringContainer = '';                 
                  stringContainer = stringContainer + decoder.write(string);
                  let recordObject = JSON.parse(stringContainer);
                  console.log('This is the userObject:' + '\n', userObject)                  
                  console.log('This is the recordObject:' + '\n', recordObject)
                  // done(function(recordObject){userObject.hashedPassword = recordObject.hashedPassword; console.log(userObject);})
                } 
                */

                console.log('I am done');
                return
              }); //End of: helpers.getHashedPass(dataObject, function(statusCode, payload, contentType)
            }; // End of: Else we need to lookup the hashed password from the previous record.
      ...
      else // The email address does NOT already exists so exit this process without appending the record.
      {      
        helpers.log
        (
          5,
          '1trx8iyyv07jk0e6spuh' + '\n' +
          'The email address: ' + email + ' does not already exists' + '\n' +
