...
handlers.userEdit = function(data, callback)
{
  ...
  if(data.method == 'get')
  {
    ...
    var templateData = 
    {
      ...
      //'head.clientCode' : 'public/app.js',
      'head.clientCode' :  'accounting/dbPermissions/user' + '/' + 'userEdit' + '.js' 
    };

    // Read in a template as a string
    helpers.getTemplate('../accounting/dbPermissions/user/userEdit', templateData, function(errorGetTemplate, str)
    {
...
handlers.userList = function(data, callback)
{
  // Reject any request that isn't a get
  if(data.method == 'get')
  {
    ...
    var templateData = 
    {
      ...
      'body.class' : 'userList',     
      'head.clientCode' :  'accounting/dbPermissions/user' + '/' + 'userList' + '.js',
      "tableLabel":"User"      
    };

    // Read in a template as a string
    helpers.getTemplate('../accounting/dbPermissions/user/userList', templateData, function(errorGetTemplate, str)
    {
      if(!errorGetTemplate && str) // If there were no errors and a template was returned
      {
        // Add the universal header and footer.
...
handlers.userAdd = function(data, callback)
{
  // Reject any request that isn't a get
  if(data.method == 'get')
  {
    ...
    var templateData = 
    {
      ...
      'body.class' : 'userAdd',      
      'head.clientCode' :  'accounting/dbPermissions/user' + '/' + 'userAdd' + '.js',      
    };

    // Read in a template as a string
    helpers.getTemplate('../accounting/dbPermissions/user/userAdd', templateData, function(errorGetTemplate, str)
    {
      if(!errorGetTemplate && str) // If there were no errors and a template was returned
      {
        // Add the universal header and footer.
                                  ...
                                  function(err)
                                  {
                                    ...
                                    else // There was an error when rolling back record for user.
                                    {
                                      ...
                                      (
                                        ...
                                        'CHECK TO SEE IF dbLog and user ARE STILL IN SYNC' + '\n' + 
...
handlers._user.put = function(data, callback){
  ...
  if(userId && (email || password || deleted))
  {
    ...
    // 1 for transactionId in dbLog to handle a possible rollback.
    // 1 because later a batchId field will be added for batch adds, updates, and deletes.
    // 1 Because the post hander uses 5 and we don't want a collision.
    // Now that we know it must be the same everywhere I will remove the parmeter.
    let idRange = 5;
    ...
    _data.nextId(idRange, function(error, nextIdObject)
    {
      ...
      if(!error && nextIdObject)
      {
        ...
        dataObject.path = '/dbPermissions/user/user.json';
