...
// Define the handler function for listing all the users.
// This function serves up a list of users to the client.
// This function is executed when the user askes for the following path: /users/list
handlers.usersList = function(data, callback)
{
  // Reject any request that isn't a get
  if(data.method == 'get')
  {
    ...
    var templateData = 
    {
      'head.title' : 'List All Users',   
      'body.class' : 'usersList',
    };

    // Read in the HTML template as a string
    // Calling getTemplate and then running the callback defined below.
    helpers.getTemplate('../accounting/dbPermissions/dbUsers/dbUsersList', templateData, function(errorGetTemplate, str)
    {
      ...
      {
        // Add the universal header and footer to the template string.
        // Calling addUniversalTemplates then running the callback defined below.
        helpers.addUniversalTemplates(str, templateData, function(errorAddUniv, str)
        {
          if(!errorAddUniv && str) // if no error and template was returned:
          {
            // Return that page as html
            callback(200, str, 'html');
          } 
...
}; // End of: handlers.usersList = function(data, callback){...}
// End of: Define the handler function for listing all the users.




// Define the handler function for creating users.
handlers.userCreate = function(data, callback)
{
  // Reject any request that isn't a get
  if(data.method == 'get')
  {
...
}; // End of: handlers.userCreate = function(...
// End of: Define the handler function for creating users.
...
handlers.favicon = function(data, callback)
{
  ...
  if(data.method == 'get')
  {
    ...
    helpers.getStaticAsset('favicon.ico', function(errorGetStaticAsset, data)
    {
...
handlers._users.post = function(data, callback)
{
  ...
  if(email && password)
  {
    let emailIsUnique = true; // Will toggle this to false if we find the email address already exists in dbUsers.
    ...
    readInterface.on('line', function(line) 
    {
      ...
      // Several records with the supplied email address may exist already if the record has 
      ...
      // be several pairs of records in the dbUsers table currently.
      // The table can be packed occasionally to get rid of these deleted record pairs. 
      // Deletes are handled as appends with a deleted flag because real deletes tie up the table for a long time.
      // It is ok to append a record with this same email address again if the last instance of this
      // record encountered in the stream has the deleted flag set to true.            

      // As explained above, only the last matching record matters.
      ...
      if (email == lineObject.email) // we found a matching entry
      {
        ...
        {
          emailIsUnique = false; // This flag used in the on close event listener below. 
        }
    ...
    {
      if (emailIsUnique) // The email address is unique so proceed with append operation.
      {
        // Hash the password
          ...
          _data.nextId(idRange, function(error, nextIdObject)
          {
            ...
            if(!error && nextIdObject)
            {
              ...
              var logObject =
              {
                ...
                "rollback" : false,
                "process" : "handlers._users.post",
                "comment" : "Append new record",
                "who" : "Function needed",    
                "dbUsers" : userObject   
              }
                      ...
                      function(err)
                      {
                        ...
                        else // There was an error appending to dbUsers.
                        {
                          ...
                          logObject =
                          {
                            ...
                            "rollback" : true,
                            "process" : "handlers._users.post",
                            "comment" : "There was an error during append. Deleting the record if any.",                        
                            "who" : "Function needed",    
                            "dbUsers" : userObject   
                          }
...
{ 
  ...
  let queryArray;
  // If the user created a query expression.
  if (typeof(Object.keys(data.queryStringObject)[0]) != 'undefined')
  {
    // Get the string which is the single element in the array which we extract 
    // from the queryStringObject using the Object.keys() method.
    let queryString = Object.keys(data.queryStringObject)[0];
    ...
    console.log("The queryArray is as follows:" + "\n", queryArray);
    ...
    if(firstQueryElement == 'WHERE') // The user filled out a where clause.
    {
      ...
      let indexOfNextPossibleANDWHERE = amountOfWhereClauses * 4
      let proceedWithLoop = true;

      while (indexOfNextPossibleANDWHERE < lengthOfQueryArray -1 && proceedWithLoop == true) 
      {
        if(queryArray[indexOfNextPossibleANDWHERE] == "ANDWHERE")
        {
          amountOfWhereClauses = amountOfWhereClauses + 1;
          indexOfNextPossibleANDWHERE = amountOfWhereClauses * 4
        }
        else
        {
          indexOfNextPossibleOrderBy = indexOfNextPossibleANDWHERE
          proceedWithLoop = false;
        }
      }

      console.log("The amountOfWhereClauses is: ", amountOfWhereClauses);
    }
    ...
    console.log("The amountOfOrderByClauses is: ", amountOfOrderByClauses);
  ...
  readInterface.on('line', function(line) 
  {
    ...
    if(lineValueObject.deleted === true) // if the record in the file dbUsers.json had been marked for deletion:
    {
      // Remove this record from the map 
      usersMap.delete(userId);
      recordWasDeleted = true;
    }
    else if(amountOfWhereClauses > 0)
    {
      let whereClauseCount = 1;
      let index = 1;
  
      while(whereClauseCount <= amountOfWhereClauses)
      {
        if(queryArray[index + 1] == 'MatchesExactly')
        {
        ...
        if(queryArray[index + 1] == 'MatchesExactlyNotCaseSensitive')
        {
          if(lineValueObject[queryArray[index]].toLowerCase() != queryArray[index + 2].toLowerCase())
          {
            // Remove this record from the map 
            usersMap.delete(userId);
            recordWasDeleted = true;
        ...
        if(queryArray[index + 1] == 'DoesNotMatchExactly')
        {
          if(lineValueObject[queryArray[index]] == queryArray[index + 2])
          {
            // Remove this record from the map 
        ...
        if(queryArray[index + 1] == 'DoesNotMatchExactlyNotCaseSensitive')
        {
          if(lineValueObject[queryArray[index]].toLowerCase() == queryArray[index + 2].toLowerCase())
          {
            // Remove this record from the map 
        ...
        if(queryArray[index + 1] == 'BeginsWith')
        {
          if(lineValueObject[queryArray[index]].indexOf(queryArray[index + 2]) != 0)
          {
            // Remove this record from the map 
            usersMap.delete(userId);
            recordWasDeleted = true;
        ...
        if(queryArray[index + 1] == 'BeginsWithNotCaseSensitive')
        {
          if(lineValueObject[queryArray[index]].toLowerCase().indexOf(queryArray[index + 2].toLowerCase()) != 0)          
          {
            // Remove this record from the map 
            usersMap.delete(userId);
            recordWasDeleted = true;
        ...
        if(queryArray[index + 1] == 'DoesNotBeginWith')
        {
          if(lineValueObject[queryArray[index]].indexOf(queryArray[index + 2]) == 0)
          {
            // Remove this record from the map 
        ...
        if(queryArray[index + 1] == 'DoesNotBeginWithNotCaseSensitive')
        {
          if(lineValueObject[queryArray[index]].toLowerCase().indexOf(queryArray[index + 2].toLowerCase()) == 0)          
          {
            // Remove this record from the map 
        ...
        if(queryArray[index + 1] == 'Contains')
        {
          if(lineValueObject[queryArray[index]].indexOf(queryArray[index + 2]) == -1)
          {
            // Remove this record from the map 
            usersMap.delete(userId);
            recordWasDeleted = true;
        ...
        if(queryArray[index + 1] == 'ContainsNotCaseSensitive')
        {
          if(lineValueObject[queryArray[index]].toLowerCase().indexOf(queryArray[index + 2].toLowerCase()) == -1)          
          {
            // Remove this record from the map 
            usersMap.delete(userId);
            recordWasDeleted = true;
        ...
        if(queryArray[index + 1] == 'DoesNotContain')
        {
          if(lineValueObject[queryArray[index]].indexOf(queryArray[index + 2]) > -1)
          {
            // Remove this record from the map 
        ...
        if(queryArray[index + 1] == 'DoesNotContainNotCaseSensitive')
        {
          if(lineValueObject[queryArray[index]].toLowerCase().indexOf(queryArray[index + 2].toLowerCase()) > -1)          
          {
            // Remove this record from the map 
        ...
        if(queryArray[index + 1] == 'EndsWith')
        {
          if(!lineValueObject[queryArray[index]].endsWith(queryArray[index + 2]))
          {
            // Remove this record from the map 
            usersMap.delete(userId);
            recordWasDeleted = true;
        ...
        if(queryArray[index + 1] == 'EndsWithNotCaseSensitive')
        {
          if(!lineValueObject[queryArray[index]].toLowerCase().endsWith(queryArray[index + 2].toLowerCase()))
          {
            // Remove this record from the map 
            usersMap.delete(userId);
            recordWasDeleted = true;
        ...
        if(queryArray[index + 1] == 'DoesNotEndWith')
        {
          if(lineValueObject[queryArray[index]].endsWith(queryArray[index + 2]))
          {
            // Remove this record from the map 
        ...
        if(queryArray[index + 1] == 'DoesNotEndWithNotCaseSensitive')
        {
          if(lineValueObject[queryArray[index]].toLowerCase().endsWith(queryArray[index + 2].toLowerCase()))
          {
            // Remove this record from the map 
        ...
        if(queryArray[index + 1] == 'IsGreaterThan')
        {
          if(lineValueObject[queryArray[index]] <= queryArray[index + 2])
          {
            // Remove this record from the map 
        ...
        if(queryArray[index + 1] == 'IsGreaterThanNotCaseSensitive')
        {
          if(lineValueObject[queryArray[index]].toLowerCase() <= queryArray[index + 2].toLowerCase())
          {
            // Remove this record from the map 
        ...
        if(queryArray[index + 1] == 'IsGreaterThanOrEqualTo')
        {
          if(lineValueObject[queryArray[index]] < queryArray[index + 2])
          {
            // Remove this record from the map 
        ...
        if(queryArray[index + 1] == 'IsGreaterThanOrEqualToNotCaseSensitive')
        {
          if(lineValueObject[queryArray[index]].toLowerCase() < queryArray[index + 2].toLowerCase())
          {
            // Remove this record from the map 
        ...
        if(queryArray[index + 1] == 'IsLessThan')
        {
          if(lineValueObject[queryArray[index]] >= queryArray[index + 2])
          {
            // Remove this record from the map 
        ...
        if(queryArray[index + 1] == 'IsLessThanNotCaseSensitive')
        {
          if(lineValueObject[queryArray[index]].toLowerCase() >= queryArray[index + 2].toLowerCase())
          {
            // Remove this record from the map 
        ...
        if(queryArray[index + 1] == 'IsLessThanOrEqualTo')
        {
          if(lineValueObject[queryArray[index]] > queryArray[index + 2])
          {
            // Remove this record from the map 
        ...
        if(queryArray[index + 1] == 'IsLessThanOrEqualToNotCaseSensitive')
        {
          if(lineValueObject[queryArray[index]].toLowerCase() > queryArray[index + 2].toLowerCase())
          {
            // Remove this record from the map 
    ...
    //If the records was not marked for deletion and passed through the filter above:
