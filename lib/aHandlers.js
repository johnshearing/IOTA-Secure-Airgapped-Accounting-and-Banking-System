...
{ 
  ...
  if (typeof(Object.keys(data.queryStringObject)[0]) != 'undefined')
  {
    ...
    if(firstQueryElement == 'WHERE') // The user filled out a where clause.
    {
      ...
      let amountOfWhereClauses = 1; // We know about the first one so far.
  ...
  readInterface.on('line', function(line) 
  {
    ...
    /*
    else if(lineValueObject.email !== email) // if the email address of this record does not match the users query
    {
      // Remove this record from the map 
      usersMap.delete(userId);
    }
