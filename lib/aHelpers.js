...
// Define a function to retrieve the hashed password from the most current record in dbUsers.json.
// Streams the most current record to the calling function.
helpers.getHashedPass = function(data, callback)
{
  ...
  {          
    ...
    if(usersMap.size === 0)
    {
      ...
      (
        4,
        'rp0pa5fw2hnatm2wdtdq' + '\n' +
        'No record found for userId' + '\n'
      );
            //params:  error 
      return callback("No record found for userId"); // Don't change this error message. Other functions may depend on it.
    }
...
}; // End of: helpers.getHashedPass = function(data, callback){...}
// End of: // Define a function to retrieve the hashed password from the most current record in dbUsers.json.
...
helpers.getStaticAsset = function(fileName, callback)
{
  ...
  if(fileName) // If fileName is a type string and is not empty
  {
    ...
    fs.readFile(publicDir + fileName, function(errorReadFile, data)
    {
