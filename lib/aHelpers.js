...
// This is for automatic logging to the activityLog only.
...
helpers.log = function(logLevel, message) 
{
  if (logLevel >= config.logLevel)
  {
    ...
    { 
      ...
      let wLogStream = fs.createWriteStream(helpers.baseDir + 'dbLogs' + '/' + 'activityLog' + '.log', {flags : 'a'});
...
// Define a function to retrieve the most current record in dbUsers.json.
// Streams the most current record to the calling function.
helpers.getMostRecent = function(data, callback)
{
  ...
  // Create a variable to track whether or not the candidate email address has already been used.
  let emailAddressAlreadyTaken = false;

  // Create a variable to track the primary key of a record that we may encounter which 
  // is holding the candidate email address.
  // If we encounter this userId again as we proceed through the records then we will 
  // check to see if the email has been changed or if the record has been deleted. 
  // If so we will set this variable to false signifying that the email address is available again.
  let userIdOfRecordHoldingCandidateEmail = false

  // This function sets up a stream where each chunk of data is a complete line in the dbUsers file.
  let readInterface = readline.createInterface
  (
    { // specify the file to be read.
      input: fs.createReadStream(helpers.baseDir + '/dbPermissions/dbUsers' + '/' + 'dbUsers' + '.json'),
    }
  );
  ...
  readInterface.on('line', function(line) 
  {
    ...
    {
      // If this record was holding our candidate email address:
      if(lineValueObject.userId === userIdOfRecordHoldingCandidateEmail) 
      {
        // The record holding our candidate email address has been deleted so...

        // The candidate email address is available.
        emailAddressAlreadyTaken = false;

        // There is no more reason to track this record.
        userIdOfRecordHoldingCandidateEmail = false;          
      }
    ...
    { // If the current record does not have a primary key matching the record we wish to change:
      ...
      {
        // Check if this record has the same email address we wish to write. Is the email address already taken?
        if(lineValueObject.email === data.email)
        {
          // Currently this email address is taken. 
          // As we proceed, we may encounter a record with this same primary key where the email has been changed.
          // Or, with this same primary key that has been deleted. 
          // Either case will make the email address available again as we proceed through the records
          // So we need to compare the primary key of this record to other records that we encounter.

          // Flag that the email address is already being used.
          emailAddressAlreadyTaken = true;

          // Take note of the userId so that we can check as we proceed if this record gets deleted or if the email address gets changed.
          userIdOfRecordHoldingCandidateEmail = lineValueObject.userId;
        } // End of: Check if this record has the same email address we wish to write. Is the email address already taken?

        // Well then - Not deleted, not the same key as the record we want to change, not the candidate email address so...
        // Check if this record was previously holding the candidate email address but is now changed.
        else if
        (
          userIdOfRecordHoldingCandidateEmail === lineValueObject.userId 
          &&
          lineValueObject.email != data.email 
        )
        {
          // This record was tying up the candidate email address but is no longer holding it.
          // The candidate email address is available again.
          emailAddressAlreadyTaken = false;
    ...
    // If the record was not marked for deletion and has the userId of the record we wish to change:
  ...
  {          
    ...
    if(usersMap.size === 0)
    {
      ...
      (
        ...
        'No record found for userId' + '\n'
      );
            //params:  error 
      return callback("No record found for userId"); // Don't change this error message. Other functions depend on it.
    }

    if(emailAddressAlreadyTaken === true)
    {
      helpers.log
      (
        4,
        ...
        'This email address already exists' + '\n'
            ...
            //params:  error 
      return callback("This email address already exists"); // Don't change this error message. Other functions depend on it.
    }
...
// End of: // Define a function to retrieve the most current record in dbUsers.json.
