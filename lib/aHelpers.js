...
{
  // This prevents an error in case there is no unique field to validate.
  if(data.uniqueField01Value === undefined) 
  {
    data.uniqueField01Value = "";
  }
  ...
  {          
    ...
    // Notify calling function if the specifed record was not found, then stop processing
    if(tableMap.size === 0)
    {
      helpers.log
      (
      ...
      sourceStream.push(null);     
            ...
            //params:  error                             empty stream
      return callback("No record found for primary key", sourceStream);
    }
    
    // Notify calling function if the unique key specifed is already in the table, then stop processing.
    if(uniqueValuesAlreadyUsed === true)
    {
      helpers.log
      (
        4,
        'nwrv16fc90bzrfnvhbg8' + '\n' +
      ...
      sourceStream.push(null);     
            ...
            //params:  error                                                       empty stream
      return callback('This ' +  data.uniqueField01Name + ' value already exists', sourceStream);
    }

    // No issues and we have the most recent data for the primary key supplied so return the data to the calling function.
...
// Define a function to check that a string has a number.
helpers.hasANumber = function(checkString) 
{ 
  ...
  return false; 
}; // End of: Define a function to check that a string has a number.
