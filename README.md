...
* A Code Generator:
  * Rather than writing code such as: Post, Get, Put, Delete, Validation, page handlers, HTML and client side javascript for every table in the database, a program called **the formula**, which contains both template snippets and instructions, reads metadata from a data dictionary which contains code snippets, data definitions, the business rules that make each table, each record, and each field unique. Then the formula assembles all this information into working software that is the accounting/banking system. Google defines the word **schema** as a representation of a plan or theory in the form of an outline or model. So it seems resonable to call this method of creating software **Schema Oriented Programming**.  
