...
* [Generate addresses and check balances for a single address or for a list of addresses](https://github.com/johnshearing/IOTA-Airgapped-NodeJS-Console-Wallet#check-your-balance-at-address-a0),   
  * The accounting system does NOT check seed balances directly. This is for security and performance reasons.  
  * Instead, a list of addresses is generated from behind the airgap by the seed and is supplied by the airgapped database to the online database. The balances of each address can be checked individually by the online database which queries the Distributed Public Ledger and adds them up to give the seed's balance if required. This way you can get the seed balance without exposing your seed to any other device.  
  * Working this way also eliminates the problem of false balances created by snap shots in the case of IOTA or a block of 21 or more empty addresses in the case of Cardano.  
  * It should not be necessary however to go to the Distrubuted Public Ledger in order to get the balance for a seed in the case of IOTA or master key in the case of Cardano. The online database/accounting system which broadcasts all transactions keeps track of the account balances. It works as follows: When your customer is ready to pay, an address is provided by you (the seller). The buyers wallet would use the provided address to generate encrypted transaction instructions as usual. But instead of broadcasting the encrypted instructions to the Distributed Public Ledger they would send the instructions to the seller who generated the addresses. The seller would broadcast the transaction from his online database/accounting system and make note of the amount in the new address. Now the seller can see the balance of his account by looking in his own database without ever needing to query the Distributed Public Ledger. This gets rid of the problem with spanning empty addresses, eliminates the need to dust, reduces bloat, reduces the need for bandwidth, and increases performance by a factor of thousands. Think of it this way: The controller of a company does not ask the bank how much he has in his bank account - he already knows from looking in his own accounting system. In the same way a modern accounting system for cryptocurrency will track balences without needing to query the blockchain.
